...
- **A peer's version on the Updates page refreshes on its own.** It refreshes after a crew update, with no pull to refresh. A run that levels only the peers writes no record on the lead, and the page used to drop that run entirely and stop asking for a fresh census.
