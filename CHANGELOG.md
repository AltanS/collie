...
- **Attention shows only the panes that need you.** Each workspace keeps only its panes that are blocked or ready and unseen, and a workspace with none drops out. Nothing is sorted: workspaces and panes keep their order, each heading still counts its whole workspace, and the summary line still counts every pane. When nothing needs you, the tab shows the "Nothing needs you" line instead of an empty list. The tab shows a count while something needs you (ADR 0066). Thanks @simplysoft (#270).
