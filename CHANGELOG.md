...
- **The pane's two chrome rows fold into one bar of beads.** A chevron at the tab row's trailing end collapses the tab strip and the pane strip together into a 32px bar: one bead per tab in the same triage palette the tabs use, one per pane, the current one ringed — so "how many, where am I, is anything shouting" survives the fold and a tap anywhere on the bar puts the rows back. The choice is remembered per device. The soft keyboard folds them on its own whatever the preference says, and an expand taken while it is up lasts only as long as the keyboard does
