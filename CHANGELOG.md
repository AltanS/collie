...
- **Every dialog card has a way back to the terminal.** A Terminal control on the card shows the rows it replaced; Back returns to the card. The choice lasts as long as the dialog (ADR 0056).
- **The photo picker takes several photos at once.** Pick more than one photo in the attach sheet and each one uploads in turn, in the order picked, through the same size and type checks as a single photo. The Files picker is unchanged. Thanks @jyothyswaroop (#259).
...
- **The composer is one box.** The attach control, the text field and Send sit on one row inside one bordered container, the pattern the shadcn-registry prompt kits use, ported by hand with no new dependency. An empty composer is one button row tall, the buttons stay pinned to the bottom edge as a long draft grows, and focus draws one frame on the whole box. The 44px strip the attach button held inside the field goes back to the typing area. Ctrl+Enter still sends and a bare Enter is still a newline, because the draft goes to a terminal (ADR 0057).
...
- **The Keys tray has a tall Enter, set apart from the arrows.** Enter spans both rows at the right edge behind a small gap and carries a light tint, the arrows form an inverted T, and Space, Shift, Tab and the arrows show icons. Thanks @lighcen (fixes #263).
...
- **The folder trust prompt is two buttons again in current Claude Code.** Claude Code 2.1.278 prints the trust prompt's rows without numbers, so Collie declined it and offered only the unread-dialog card's Escape. The prompt lifts again: because the rows lost their numbers, a tap now walks the pointer with the arrow keys and confirms with Enter, exactly the keys the screen names, and no digit is invented. The option under the pointer is "No, exit", which quits Claude, so the card marks the row the pointer is on and every other row shows the arrow its tap starts with. The older numbered prompt is unchanged (ADR 0055).
- **Resuming a Claude Code session works from the card.** The /resume picker lists each past session as a row; a tap moves the pointer there and resumes it. The picker never prints Enter, so this dialog alone sends it unprinted (ADR 0058).
- **The dashboard keeps its scroll position when you open a pane and come back.** Opening a pane and returning used to always land back at the top, because the screen transition remounts the dashboard's own scroller as a fresh element. The dashboard and the space screen now each remember their own scroll position in memory and restore it on the way back.
- **The Keys tray's Enter really spans both rows, and Ctrl+C fits as ^C.** A shared fixed height silently overrode Enter's two-row span, so it drew as a single 36px row with its icon and caption pressed together; the grid now reserves two explicit 36px rows and Enter stretches to fill both. The quick Ctrl+C key read "Ctrl C", wider than its 1/7 column on a 390px phone, and spilled past its edge; it now shows "^C", still sends `ctrl+c`, and keeps "Ctrl+C" as its accessible name. Every pad key also gets a truncating label so this class of overflow cannot recur.
