...
- **The actions row is a belt.** One full-bleed band above the input, flush under the mirror, carries Collie's controls on its ground and the harness's commands in a section tinted with the harness's colour, so the row reads as one strip with two parts rather than two floating capsules.
...
- **A tab with no name of its own shows no number.** A multiplexer labels an unnamed tab by position, "1" or "Tab #2"; that number is now treated as no name at all, so the tab strip and the space view draw a small dot in its place and the space stands alone in every crumb. A screen reader still hears the label.
...
- **The dashboard's rows sit under the space and tab they live in.** What needs you stays on top, by urgency, with each row still carrying its place; every other pane now sits under a heading that names its `space › tab` and counts what is inside it, in the multiplexer's own space and tab order. A row under such a heading drops its second line, because the heading already said it, and every row of a group is the same height. Bare shells join their tab's group after its agents. The Working and Recent headings are gone with them, and so are Recent's fold and its sort toggle.
