...
- **A peer's spaces and tabs appear on the phone.** The merged snapshot host-tagged panes and sessions but not workspaces or tabs, and Herdr numbers those per machine — so two default installs both called theirs `w1` and `w1:t1`, the member's space and tab had no row at all, and every count on the surviving row was the lead's. A pack of two machines with one pane each rendered as one space claiming one pane. `(host, workspaceId)` is the identity now; the space navigator follows the host you are pointed at, and a solo snapshot is unchanged to the byte (3159780)
- **An unreachable member is described in Collie's words, not Bun's.** *"Unable to connect. Is the computer able to access the url?"* — a browser console's sentence — reached `pack status`, the 503 body a phone reads and `collie leave`'s warning. Each failure now says what the far side did: nothing accepted a connection, the address does not resolve, the TLS certificate was not accepted. A message carrying a duration keeps it, and an unrecognised failure is passed through rather than dressed up (8acdd2e)
- **`pack add`'s port probe says what it saw, not what is true.** It reported `✓ port 8787 free` over a service that was crash-looping — true at that instant, and a coin flip against a five-second restart cycle. The negative answer now reads "nothing was listening just now". Whether a port is held by a service that keeps coming back is not knowable from the lead, and the probe says so instead of pretending (cf0d2fa)
- **The v1 beta install command resolves the newest tag instead of pinning one two releases stale**, and `## Install` now says in one line that the bare command tracks the stable line (eebbb4b)
- **PACK_PROTOCOL.md §9.2 and the registry comment call the wire parameter `host=`, which is what the code sends.** Both said `?h=`, which is the browser route's spelling only — and because an absent host means "this collie", a hand-written `?h=member` does not 404, it silently addresses the lead (befa1b0)
- **A long upload path can no longer push Send off the screen.** Attaching an image appends the bridge's host path to the draft as one unbroken token, which laid the composer row out 575px wide inside a 378px phone frame and put Send 209px past the right edge (53ba1a7)
- **The composer's notices slide in instead of shoving the field up.** Six in-flow strips were bare conditionals, so each one teleported the whole composer up by its own height the moment its condition flipped — worst with the keyboard up and the eye in the box. All six now arrive and leave through the app's one collapse surface, and the two armed-mode strips share a single slide rather than fighting for the row. The sort was checked too: every strip left in the footer is a condition carrying its own controls, and the one real event there was already a top pill (c676583)
- **A space row's time sits left of its pane count, like every other list in the app.** The count chip now anchors the right edge whether or not a row has a timestamp, so rows with and without one line up (a846450)
- **A pane's hint truncates instead of wrapping.** One long sentence made its card taller than every other row in the list, for no reason a glance can see; the full text stays one tap away in the pane view, and a hover away on desktop (dac6b35)
- **`pack status` on a peer no longer reports a healthy lead as a starved data link.** It asked every member two questions — `hello`, then a real snapshot read — which is exactly the poll a lead runs. But a peer's one roster entry is its LEAD, and the snapshot route is deliberately not on the closed peer→lead set (PACK_PROTOCOL.md §8.6: `leave`, `lead`, `hello`, the warrant deliveries). So the lead refused, correctly, and the peer rendered the refusal as `data STARVED … not arriving inside the per-poll budget` over a remedy — raise two timeouts — that cannot move an authorization refusal. A peer now asks its lead one question and rests the row on it; `collie doctor`'s `lead-reach` reports what it actually asked (c442daf)
- **`collie leave` ends on the truth about the machine it just fixed.** The closing health banner was built from the environment as this run READ it, not as `leave` had just rewritten it, so a correct tear-down finished with `⚠ Collie isn't answering on 192.168.77.2:8787 yet` about a machine that was healthy on loopback — the same alarm the crash-loop fix used to raise, now false, at the end of the same command. The banner also resolves its probe target once, so it and the `local` row can never name two addresses, and a wildcard bind probes loopback (4829d07)
- **Re-adding a machine that left works without a terminal.** `collie leave` unsets `COLLIE_HOST`, so `pack add` on that machine found nothing and asked `is configured to bind (unset):8787` — a placeholder in operator copy, and self-defeating, since an absent value is precisely what says there is nothing to preserve. Non-interactively it stopped dead, `ssh -tt` included. An unset bind is now written straight through, with the reason said out loud; a bind somebody set, wide ones included, is still confirmed before it is replaced (f7c9d1b)
- **A peer stops offering a front door it does not have.** The start banner still printed a `tailnet` row — a door that is not there, at a loopback URL that is not even a peer's bind — and `collie serve` typed by hand ended on `open: http://127.0.0.1:8787` one line under the sentence saying a peer publishes none. The refusal now stands alone, and the banner names the pack instead, because on a peer the door to point a phone at is the lead's (1c30937)
