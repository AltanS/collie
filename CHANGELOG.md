...
- **The machine's name moves into the pane header.** On a crew the host tag sits under the cache reading at the header's right edge, right-aligned with the menu, in the same two-corner layout the dashboard rows use, and the actions belt no longer carries it.
