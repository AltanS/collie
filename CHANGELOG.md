...
### Added
- **A pane can show what changed in its workspace since the last commit.** Tap the Changes mark at the right end of the actions belt, beside the pane switcher: the files changed in the git repos of the pane's workspace folder, and in repos inside it, grouped by repo with added and removed line counts. Tap a file to read its diff, then step to the next file from there. It only reads: nothing is staged or committed, a repo's own hooks and filters never run, and it never reaches the network, not even to fetch a file a partial clone left out. Settings → Changes decides whether it looks inside the folder and how deep. zellij panes show no Changes mark, because zellij reports no folder (ADR 0065). Thanks @lighcen.
- **The Changes list can show as a folder tree, and a filter narrows it.** Toggle List or Tree at the top; the Filter button matches files by path and by status letter, and Previous and Next step through only the files shown. The filter opens as a card floating over the list rather than a row that pushes it down, and a tap outside it or Escape closes it without clearing it.
- **A diff in the Changes view shows syntax colour.** TypeScript, JavaScript, Python, Go, Rust, shell, YAML, JSON, CSS, Markdown and 16 more languages are coloured once the highlighter loads, which happens only when you open such a file. The plain text shows first and nothing moves when the colour arrives. A diff over 2000 lines stays plain (ADR 0065).
- **Every pane in a workspace shows the same Changes list, and the header names the workspace and its folder.** The folder is the workspace's own when herdr or tmux keeps one, else the folder that holds all its panes, never your home folder or above it; past that the pane's own folder stands in. `GET /api/workspace/<id>/changes` asks for the same list without naming a pane, forwarded across a crew like the pane route. Opened from a pane, the list marks the repo that pane works in with "This pane" and scrolls to it, and when repos sit deeper than Settings lets it look, the list ends with a note that links to that setting (ADR 0065).
- **The dashboard has a footer with three tabs: Panes, Focus and Changes.** Panes is the dashboard as it was. The tab you pick is kept on this device, and the footer, the workspace strip and the summary line stay put when you switch (ADR 0066).
- **Focus shows only the panes that need you.** Each workspace keeps only its panes that are blocked or ready and unseen, and a workspace with none drops out. Nothing is sorted: workspaces and panes keep their order, each heading still counts its whole workspace, and the summary line still counts every pane. When nothing needs you, the tab shows the "Nothing needs you" line instead of an empty list. The tab shows a red count only for panes blocked on you. When none is blocked but a finished pane waits unseen, it shows a quiet dot and no number (ADR 0066). Thanks @simplysoft (#270).
- **The Changes tab lists every workspace with its changed files.** Each row shows how many files changed and the lines added and removed, and a tap opens that workspace's Changes. A clean workspace shows "No changes" and a zellij workspace "No folder". The counts are read the moment you open the tab or come back to the app, refresh every 5 seconds while the tab is open, and stop when you leave it or the app is hidden (ADR 0066).
- **An open Changes screen refreshes itself every 5 seconds.** The list, and the diff of an open file, are read again while the app is visible, and nothing moves when nothing changed. A file that is no longer changed says so in place, and after two failed reads in a row the header says "Not updating". Phones and tabs that read the same workspace at the same moment share one git run (ADR 0065).
- **Changes can show a repo's last commit.** Agents commit their own work, so the list often empties right after the change you wanted to read. A clean repo now offers "Show last commit": its subject, short hash, author and age, then its files with the same list, tree, filter and diffs. A newer commit or new uncommitted work shows as a quiet line to tap, never swapping what you read. It reads HEAD only, with the same hardening as the rest of the view (ADR 0065).
...
- **The actions belt is 15% larger, and Settings can make it larger still.** The band, its pills, icons and words grow together: the band is 45px, up from 40, and the icons 18px, up from 16. Settings → Action belt size offers Default, Large and Larger.
...
- **Older strings that still read English in those six languages are translated too.** The harness bar, spaces, notification and pane settings, the prompt-cache sheet, the first-run tour and the update screen predate 1.12.1 and had never been translated.
- **Tapping a workspace in the Changes tab carries its name and counts into the Changes screen.** The Changes list's header now shows the workspace as its title with the file count and +added −removed under it, right from the first frame, and where the browser supports view transitions (Safari 18, Chrome) the row's name and counts glide into place. A first visit shows placeholder rows until the list arrives, then fades them in; a revisit shows the last list at once. The swipe back and reduced motion skip the glide.
- **The Changes screen's back arrow carries the name and counts back into the tab row.** Going back to the dashboard's Changes tab, they glide from the header down into the row you tapped, and when that row is out of sight the two screens crossfade instead. The swipe back and reduced motion skip it, as on the way in.
- **Opening a pane carries its row into the pane's header, and the back arrow carries it back.** A tap on a pane in the dashboard or in a workspace glides the row's status dot, agent icon and name into the header, where the browser supports view transitions (Safari 18, Chrome); the back arrow glides them down into the row you tapped, or crossfades when that row is out of sight. The pane's first read now starts as your finger lands, so the screen usually opens a little sooner. On a slow link the pane opens with the usual slide instead, never a frozen screen. The swipe back and reduced motion skip the glide.
...
- **The update screen no longer closes while the other machines still update.** It used to close, and say "Crew updated", the moment the lead finished, while a member was still waiting its turn. It now stays on the members' step until each one has arrived or you skipped it (ADR 0064).
- **This phone reloads once, at the end of an update, and comes back to the update screen.** The reload used to happen whenever a new app arrived, even in the middle of the run, and the page that came back had forgotten which update it was showing. It now waits for the phone's own step, and the update screen reopens after the reload at the step it had reached. A new app that arrives while you have unsent text now waits for that text too, instead of reloading over it (ADR 0064).
- **The "done" message no longer repeats every time the app loads.** It showed again on every load for ten minutes after an update. The Done screen replaces it, on the phone that started the update only (ADR 0064).
- **"Try minibuch again" names the machine.** A retry for one member used to say "Retry crew update". The button and the screen it opens now name the machine the retry is for.
- **A drag-opened pane switcher no longer drops out and slides back in.** Opening the switcher with the upward drag on the belt sometimes showed it open, vanish and open again, when the pane redrew while the switcher was up. It now stays put, and a tap-open still slides in. Thanks @enieuwy.
