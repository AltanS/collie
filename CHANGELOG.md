...
- **Every pane in a workspace shows the same Changes list, and the header names the workspace and its folder.** The folder is the workspace's own when herdr or tmux keeps one, else the folder that holds all its panes, never your home folder or above it; past that the pane's own folder stands in. `GET /api/workspace/<id>/changes` asks for the same list without naming a pane, forwarded across a crew like the pane route (ADR 0065).
- **The dashboard has a footer with three tabs: Panes, Attention and Changes.** Panes is the dashboard as it was. The tab you pick is kept on this device, and the footer, the workspace strip and the summary line stay put when you switch (ADR 0066).
- **Attention shows only the panes that need you.** Each workspace keeps only its panes that are blocked or ready and unseen, and a workspace with none drops out. Nothing is sorted: workspaces and panes keep their order, each heading still counts its whole workspace, and the summary line still counts every pane. When nothing needs you, the tab shows the "Nothing needs you" line instead of an empty list. The tab shows a red count only for panes blocked on you. When none is blocked but a finished pane waits unseen, it shows a quiet dot and no number (ADR 0066). Thanks @simplysoft (#270).
- **The Changes tab lists every workspace with its changed files.** Each row shows how many files changed and the lines added and removed, and a tap opens that workspace's Changes. A clean workspace shows "No changes" and a zellij workspace "No folder". The counts refresh every 5 seconds while the tab is open, and stop when you leave it or the app is hidden (ADR 0066).
- **An open Changes screen refreshes itself every 5 seconds.** The list, and the diff of an open file, are read again while the app is visible, and nothing moves when nothing changed. A file that is no longer changed says so in place, and after two failed reads in a row the header says "Not updating" (ADR 0065).
...
- **The dashboard's second tab is called Attention.** It was "Needs you", which read as untrue when nothing needed you; the count still shows only when something does (ADR 0066).
...
- **A swipe back goes up one level.** On the installed app the edge swipe used to land on the last screen you saw: the pane you switched away from, the History you just closed, or Settings again after its back arrow. Switching panes, tabs, spaces or machines now replaces the screen instead of stacking it, and every back arrow steps back to the level it came from, so the swipe and the arrow agree. A pane opened from a notification has the dashboard behind it, a notification tapped while the app is open no longer reloads it, and a swipe no longer plays the app's own slide on top of the phone's (ADR 0067).
