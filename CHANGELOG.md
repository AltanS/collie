...
- **The cache countdown carries an hourglass mark.** The chip on the dashboard row and in the pane header opens with one fixed glyph in the chip's own ink, so the number reads as a cache reading beside the host chip's server mark rather than as a loose word; the glyph says a window is running out and leaves warm, expiring and cold to the ink.
