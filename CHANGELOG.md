...
- **A crew update takes a member straight to the new release.** A member one release behind its lead took the lead's old release the moment the update was confirmed, then refused the new one for an hour, and the Updates page said "no change for 20 minutes". The lead now hands out a turn only once it runs the new release itself. A member takes the second step of a run it already joined without waiting out its hour. And a member that is waiting on its own once-an-hour limit reads "rate-limited, retries by HH:MM" instead of failing (ADR 0062).
- **The update band no longer runs text off the edge of a phone.** Its one line now truncates with an ellipsis instead of clipping mid-word, and every state that points at `/settings/updates` — an offer, a packaged offer, a peer that could not update — makes the whole band a single 44px-tall tap target, keyboard-reachable and named by its full sentence, with its own close staying independently tappable beside it.
- **The stacking primitive behind that band, the composer's status slot and every other swap between alternatives keeps its box inside the screen.** `ui/one-of.tsx`'s grid layers had no `min-w-0`, so a `white-space: nowrap` run inside one of them — the update band's own copy, on a narrow phone — could size the whole grid column to the unbroken sentence and push it off the right edge, past the point any of that copy's own truncation could still draw an ellipsis. A real-browser case now pins the geometry a unit test cannot see.
