...
- **A Muse request for network access shows its answers as buttons.** Muse asks before it reaches a host, and Collie used to show only the card with Esc, which answers No. The four answers now show as buttons, with the host and the full address on screen above them. Thanks @jpcarranza94 (#280).
