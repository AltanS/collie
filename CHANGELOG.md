...
**This release is the whole `1.0.0-alpha` / `1.0.0-beta` line in one entry.** What follows is what
changed since the 0.x releases, grouped once; the per-release detail is in the sixty-five prerelease
entries below and this rollup does not replace them. Coming from 0.x, read
[`docs/upgrading.md`](./docs/upgrading.md) → *Upgrading from 0.x to 1.x* first, then the **Changed**
section here — that is the part that may ask something of you.

### Added

- **Collie drives tmux and zellij, not only Herdr.** Setting `COLLIE_MUX=tmux` or `COLLIE_MUX=zellij` selects the multiplexer behind one Collie-owned port (`bridge/mux/`): capabilities are declared per adapter and never inferred from a name, the evidence matrix lives in [`MUX_CONTRACT.md`](./MUX_CONTRACT.md), and a conformance suite runs against every registered adapter. The UI reads that declaration from `GET /api/config` rather than a multiplexer's name — an absent capability hides its control and explains itself in the adapter's own words, so zellij drops the space strip and declines `setFocus` while tmux keeps both. Freshness is declared as well: on a bounded mux the census is coupled to attention (every 1.5–3 s while a phone is watching), with pull-to-refresh, refresh on foreground, and `POST /api/refresh` for "look now". Herdr is now the reference adapter behind that port, its plugin route unchanged ([ADR 0022](./.adr/0022-the-mux-seam-is-a-port-collie-owns.md), [ADR 0031](./.adr/0031-freshness-is-a-declared-promise.md))
- **Agents name themselves on tmux and zellij, through their own hooks.** `collie hooks install claude` writes the guarded Claude emitter into `~/.claude/settings.json` — marker merge, symlink refusal, one-time backup, self-heal — and `collie beacon emit` is what it calls. A decorated pane then carries its agent's name, its status (`waiting` surfaces as blocked) and a session ref that pane history rides through the unmodified journal; capabilities lift only while hooks are installed. A beacon is a hint, never a control channel ([ADR 0024](./.adr/0024-a-beacon-is-a-hint-never-a-control-channel.md)). An expired beacon now leaves a pane that reads as any other shell rather than an agent stuck at "unknown" for the beacon's whole TTL, and `collie doctor` reports `beacon-hooks-claude` and `beacons` as warnings, never errors
- **A pack brings several machines to one phone.** A lead merges its peers' spaces, tabs, and panes into one snapshot and proxies every read and write byte for byte, so the phone shows one herd across machines. Enrollment relies on a spend-once invite carrying the lead's certificate fingerprint, admitted by two factors — pinned mutual TLS plus the pack secret — and `collie pack add <ssh-host>` completes the join over one multiplexed SSH connection, pushing the lead's own commit as a `git bundle`; `collie pack update <member>… | --all` levels members the same way, running over the operator's SSH and never over the pack wire ([ADR 0013](./.adr/0013-a-peer-listens-without-becoming-a-front-door.md), [ADR 0016](./.adr/0016-updates-ride-the-operators-ssh.md)). `collie pack invite | join | leave | status | rotate | remove | promote | reconnect | set-address` supply the verbs, `pack status` and `collie doctor` flag version skew alongside the `pack update` command that resolves it, and the wire contract is documented in [`PACK_PROTOCOL.md`](./PACK_PROTOCOL.md)
- **A pack overview page at `/pack`.** One read-only card per machine — health, version, secret pickup, deputy — drawn as a formation with the lead at the apex and peers fanned below; tap a machine for its detail sheet. It is fed by `GET /api/pack`, which 404s on a solo install and on a peer, and its entry points in the machine switcher, the Settings screen and the dashboard footer are hidden unless this collie is in a pack
- **A deputy takes over when the lead goes dark, on the operator's tap.** `collie pack deputy <member>` mints a signed, generational warrant, pushes it to every peer and arms them; the named machine then binds a standby door on its own `COLLIE_STANDBY_PORT`, armed only by the lead's silence and spent only by the operator's pairing credential. There is no automatic election — the operator is the quorum ([ADR 0026](./.adr/0026-the-operator-is-the-quorum.md), [ADR 0027](./.adr/0027-the-deputy-is-named-ahead-of-time.md), [ADR 0028](./.adr/0028-the-standby-door-is-a-second-listener.md)) — and a deposed lead heals into a peer instead of crash-looping. `pack status` renders the whole arming state, `collie pack set-address <member> <host:port>` repairs a row a takeover left undialable
- **Voice input — a microphone in the composer, and hands-free.** Off until you run `collie stt setup`; `collie stt test | status | off` round-trip it, report where every field came from and remove it. Speech-to-text is a registered provider seam Collie owns ([ADR 0029](./.adr/0029-speech-to-text-is-a-provider-seam-collie-owns.md)), never a special case: `openai-compatible` covers the public API, the cloud clones and a local whisper.cpp or parakeet.cpp server, which is the zero-egress choice; the `codex` provider borrows a short-lived token from the operator's own `codex app-server`. The spoken language is a setting (`--lang`, `COLLIE_STT_LANG`), blank meaning auto-detect, and hands-free sends through the guarded reply path rather than around it. Setup is a CLI act, never a web form, for the reason pairing is
- **Collie's UI in six languages — English, Deutsch, Español, 한국어, 日本語 and 中文.** Picked in Settings by native name and remembered per device, from a typed dictionary rather than a library ([ADR 0030](./.adr/0030-the-ui-is-translated-by-a-typed-dictionary-not-a-library.md)) — all six files change together and `tsc` enforces it. Every displayable refusal from the bridge carries a stable `code` and named `detail` beside its unchanged English sentence, so the phone renders even an error in its own language. The terminal mirror, agent output, quick replies and menu labels the screen itself printed stay untranslated
- **Device pairing.** `collie pair` mints a one-time code, the phone spends it in Settings for a bearer token, and every write needs that token from then on — enforced exactly while at least one device is paired, so an install that pairs nothing is unchanged. `collie devices list` and `collie devices revoke <label>` are the other half, and a revocation lands on the running service with no restart. Reads stay ungated; this gate and the older `COLLIE_DEVICE_HEADER` gate stay independent and compose by AND
- **Collie ships binaries, and installs with one command.** Every `v*` tag now publishes per-platform payload tarballs with a checksummed manifest and a `.sha256` sidecar per asset; `curl`-piping `install.sh` downloads and verifies one, lays it under `~/.local/share/collie/versions` behind a `current` symlink and links the binary onto PATH, with no sudo at any point. `collie update` detects the install kind structurally and fetch-verify-swaps with a pre-flip smoke test and post-flip auto-rollback, keeping one previous version for `collie update --rollback`; a routine update stays inside its installed major and crossing to the next needs `collie update --major` ([ADR 0020](./.adr/0020-a-major-upgrade-is-consented-by-flag.md)). A prerelease install walks its own train and takes the first strict release of its major the moment one exists, so a beta tester is never frozen on the beta they installed. `COLLIE_TAG` pins one exact release, as an environment variable rather than a flag so it survives `curl … | sh`, skipping the GitHub tags API entirely — the rescue for the day the installed binary cannot run its own `update`, which `docs/upgrading.md` → *When collie will not run* spells out beside running the previous version's on-disk binary; a rate-limited tags call now says so plainly and names the pin as the way around it. The update banner and the transcript speak the install's own language, a successful update says so when your beacon hooks are behind and names `collie hooks install claude`, and those hooks survive an update at all now — the hook command resolves through realpath to `current/bin/collie` instead of pinning a versioned path the updater's own cleanup deletes. A binary install also reads its own `~/.config/collie/.env` rather than letting a Herdr binary that happens to be on PATH claim the config dir
- **`collie doctor` — one read-only pass over the traps that fail silently.** Bind, ACL, front door, `web/dist`, the multiplexer, the beacon hooks, the journal's transcript roots and the clock locally; drift, rotation, reachability and version skew across a pack. Every finding names the verb that fixes it, one remedy per ✗, warnings exit 0 and `--json` makes the whole report scriptable. The first finding is this Collie's own version and platform, so a `doctor --json` block pasted into an issue never needs the follow-up question. Findings are scoped to the multiplexer in use: a healthy tmux or zellij host is no longer failed over Herdr checks about software it does not run
- **A new mark and a new icon family, and they move.** The mark drifts at rest (48 s/turn) and turns one full round whenever the app has something to tell you — a send landing, an agent finishing or needing you, a pane closing under you — thrown quickly and coasting down slowly, the way a spun wheel behaves; `prefers-reduced-motion` stops it entirely. The six favicons and home-screen tiles were redrawn to match, the tab favicon carrying the head on dark paper and the full tile at 96, and the accent beads read at the operator-chosen 0.7 blend point instead of the muted pair they used to rest at. Every mutating call the app can make must now name the channel that acknowledges it (`lib/ack-manifest.ts`), so a tap is answered at the control rather than on the next poll
- **The app's typeface is yours — Settings → Typeface, per device.** **System**, **Space Grotesk**, or **Aldrich**, the shipped default; `theme.toml` is a fourth operator file whose `[[font]]` rows ADD your own faces to that list, served read-only from `<config-dir>/fonts` and live on an mtime check. The choice is applied before first paint as a root class, so a pinned face never flashes the default, and it dresses chrome only — an agent's words are never in it ([ADR 0033](./.adr/0033-the-app-face-is-a-device-preference.md)). The composer's draft field is a separate setting of its own (Settings → Terminal font, 13–16, default 14) because output you scan and a sentence you are writing want different things
- **Zen mode, and chrome that gets out of the way.** **Settings → Zen mode** puts a row in the pane's ⋮ menu that takes every Collie surface off the screen and leaves the mirror alone; a floating button and Escape bring it back, and the state is deliberately transient, so a pane always opens with its chrome (#139 — thanks @abosnjakovic). Short of that, a chevron folds the tab strip and the pane strip together into a 24px bar of beads — one per tab in the tabs' own triage palette, one per pane, the current one ringed — so "how many, where am I, is anything shouting" survives the fold, remembered per device and folded by the soft keyboard whatever the preference says. The soft keyboard also stands down the pane-switch handle and the agent statusline, the mirror's bounds follow the viewport rather than one phone, the header stacks the brand over the multiplexer's name on one 60px row — the eyebrow riding above out of flow, so the host and session chips and the gear share the mux line's centre instead of floating above it — and the composer's status band sits 8px shorter on rules that no longer read as a bright sandwich
- **Work you used to walk back to the desk for.** Worktrees from the phone: the `+` on the spaces list offers a **worktree** beside a plain space, creates it through the multiplexer rather than `git worktree add` ([ADR 0032](./.adr/0032-a-worktree-is-opened-by-the-multiplexer-not-by-git.md)), opens it as its own space and nests it under the space showing its repo — Herdr declares the three capabilities, tmux and zellij decline them with their reasons (#135 — thanks @broven). "All sessions" in the session switcher is one triage list across every Herdr session on a machine, each row tagged with the session it lives in. "Show in terminal" moves the operator's own terminal to a pane, and only on that tap ([ADR 0031](./.adr/0031-freshness-is-a-declared-promise.md)). `quick-replies.toml` is the third operator file, replacing the Quick dock's shipped phrases on the panes you scope it to (#131 — thanks @fucx). Each tab carries the logo of the agent it runs when its panes agree on one. And an iPhone, where the browser never offers an install button, now gets the one line that is actually true: tap Share, then "Add to Home Screen"
- **Collie collects nothing, and that is written down.** No install events, no usage statistics, no crash reporting, no analytics — the one unprompted outbound call is the anonymous update check against GitHub's public tags API. `docs/security.md` gains a "What leaves your machine" section, and [ADR 0034](.adr/0034-collie-collects-nothing-and-opt-in-is-the-ceiling.md) records the ceiling on any future collection: explicit opt-in, off by default, and a breaking change to remove
- **Two upgrade guides that did not exist before.** `docs/upgrading.md` → *Upgrading from 0.x to 1.x* is the path for an operator, and *You run a fork* is the path for a checkout whose `origin` is not this repo — `collie update` refuses that checkout now instead of force-detaching it onto your own tags, and names the supported route: add `upstream`, fetch its tags, merge the release tag you want, rebuild, restart

### Fixed

- **tmux is parsed and driven correctly on the versions people run.** tmux 3.4 prints Collie's invisible field separator as visible text, so every listing line failed to parse and the app sat blind while the bridge said connected; the wire is un-escaped now, and a listing that parses to zero rows on non-empty output is refused as an error instead of stored as an empty herd. On tmux < 3.7, creating a tab or space while the global `window-size` is `manual` segfaults the whole server (tmux [#4849](https://github.com/tmux/tmux/issues/4849)) — Collie reads the option first and refuses with the `tmux set -g window-size latest` that clears it, rather than setting it itself
- **Four agent-parser fixes carried over from the 0.x line, so v1 is not behind stable on correctness.** Codex paints its final collaboration-mode field and separator as one dim segment, so its status row was rejected and every reply was refused with "the input box isn't on screen"; a wrapped Codex draft stranded itself the moment you sent a shift+enter onto an indented line, and did not recover on its own; oh-my-posh 18 paints the draft while the agent is working, so its ghost suggestion was read as part of your text and every reply to a busy pane stalled; and on Windows the supervised bridge registered as `powershell.exe` under an Interactive principal, appearing as a Windows Terminal tab that killed the bridge when closed — it runs under `conhost.exe --headless` now ([#140](https://github.com/AltanS/collie/pull/140), [#141](https://github.com/AltanS/collie/pull/141), [#142](https://github.com/AltanS/collie/pull/142), [#121](https://github.com/AltanS/collie/pull/121))
- **A peer can reach its own lead — the peer→lead dial no longer pins a certificate that can never be on the wire.** A lead's pack surface rides the front door, and `tailscale serve` or any conforming proxy terminates TLS before the process, so the certificate presented is the door's; every peer read its lead as `unreachable · unable to verify the first certificate`, `collie leave` always fell to its could-not-reach branch, and a deputy's takeover — whose first step is a `hello` at the lead — made every lead look dead. The lead is now dialled unpinned, decided by its role rather than by the shape of its address, while a witness stays pinned; §8.6's signature plus the pack secret keep the link two-factor
- **A peer's panes render on the phone again.** The lead re-emitted the peer's `Content-Encoding: gzip` over a body Bun had already decompressed, so every peer pane came up "(no recent output)" in the browser while curl stayed green; the peer hop is explicit identity now, and the ~20x cellular saving came back as a stream transform on the lead→phone hop instead ([ADR 0023](./.adr/0023-compression-is-hop-local-on-the-pack-link.md))
- **`url`, `status`, `serve` and `qr` defer to `COLLIE_PUBLIC_URL` wherever it is set**, instead of printing the bare tailnet name when `tailscale serve` is not on :443 (#122)

### Changed

- **The loopback gates fail closed.** Host validation is on, `COLLIE_TRUSTED_USER` rejects an absent `Tailscale-User-Login` as well as a wrong one, and a non-loopback bind refuses to start. The three opt-outs are `COLLIE_ALLOW_ANY_HOST=1`, `COLLIE_TRUSTED_USER_OPTIONAL=1` and `COLLIE_ALLOW_NON_LOOPBACK_BIND=1`, and a collie in a pack is exempt from the bind refusal by construction, never by relaxation. Uploads are typed by magic bytes rather than the client's `Content-Type`, and `.env` is held to owner-only — tightened in place on read, warned about either way, never a refusal to start. `.env` still wins over an exported variable, deliberately, but it says so now: one stderr note per shadowed variable, naming both values, so an exported `COLLIE_PORT` cannot vanish silently
- **`collie join` refuses an `http://` lead address unless `--insecure` is passed.** Over a plaintext hop the invite token and the pack secret cross in the clear, and a token-thief can self-enroll its own certificate before the honest joiner. An `http://` join that used to work now needs the flag, which owns the trusted-hop assumption. Separately, an invite in the old format — without the lead's certificate fingerprint — is refused outright rather than accepted unauthenticated
- **A lead is demoted only against a live approval minted on itself: `collie pack approve-promote <member>`.** Ten minutes, single use, `--cancel`; an unapproved `POST /pack/v1/lead` gets an honest 403 whoever is asking, and the demotion also requires the claim's fingerprint to match the pinned member's, so consent binds the certificate and not just the id. `pack rotate` still has no grace window and now warns that it has none: a peer offline at rotation misses the pickup and must re-join
- **`scripts/collie-ctl.sh` is a bootstrap shim and nothing else.** Every verb is spelled `bin/collie <verb>` and implemented once in `cli/`; the shim resolves Bun, builds the binary when the checkout has none, and execs it. Its path and every `collie-ctl.sh <verb>` spelling are frozen ([ADR 0006](./.adr/0006-update-advances-the-checkout-herdr-installed.md)), so nothing operator-facing moved — with one exception: the shim no longer sources `.env` to find Bun, so a `BUN_INSTALL` set only there must move to the environment. The CLI still reads `.env` for everything else
- **Collie introduces itself as its own product, CLI-first.** The tagline, the package descriptions and the PWA manifest say what it is — a phone UI for the AI agents in your terminal, mirroring Herdr, tmux or zellij — instead of naming it a Herdr plugin. The 1715-line README became a 262-line hub plus nine scoped pages under `docs/` (install, security, configure, commands, multiplexers, pack, voice-and-push, upgrading, troubleshooting), with `scripts/check-doc-links.ts` as the gate. And `collie start` no longer assumes Herdr: with no `COLLIE_MUX` it probes for a live Herdr socket, a running tmux server and zellij sessions, asks at a TTY, auto-selects a sole find headless, and refuses on zero or several — naming what it saw and the remedy. The answer is written to the config-dir `.env`; an explicit `COLLIE_MUX` is never second-guessed
- **Aldrich is the shipped default face, and no face is faked bold any more.** Every default mechanism moved with it — the preload, the boot splash, the pre-paint class list, the operator-font fallback — so a device that never opens the Typeface setting dresses in Aldrich at first paint with no layout shift; Space Grotesk stays a shipped choice and renders exactly as before. Aldrich ships one weight and the engine was inventing the other two, smearing 400 outlines into counters that are tight to begin with, worst at the 14px sizes the app does most of its reading at. `font-synthesis-weight: none` turns synthesis off for every face, and a survey confirmed no screen uses weight as its only cue between two states
- **The five non-English translations read like native product copy.** Every German, Spanish, Japanese, Korean and Chinese string was rewritten against the English source in an idiomatic register instead of a literal calque: 1851 strings moved, placeholders and code spans verified unchanged key by key, ten suspect rewrites rejected and left at their old text. English is untouched. The docs went through the same pass — 42 sections across README, CONTRIBUTING and the nine docs files, every code fence, heading, link target and URL verified byte-identical, eight rewrites rejected for losing one of those
