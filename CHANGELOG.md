...
- **The cache countdown carries an hourglass mark in one quiet ink per state.** The chip on the dashboard row and in the pane header opens with one fixed glyph, so the number reads as a cache reading beside the host chip's server mark rather than as a loose word; the hourglass is green while the window is wide, red in its last quarter and blue once it is out, the whole meta run on the pane header's path line now stands on one baseline, so both glyphs' feet and both words sit on the same line, and the machine's identity tint is left to the host chip beside it.
...
- **The actions row is a belt.** One full-bleed band above the input, flush under the mirror, carries Collie's controls on its ground and the harness's commands in a section tinted with the harness's colour, so the row reads as one strip with two parts rather than two floating capsules. The scrolling part of the belt now takes a faint brand tint and the fixed Switch cell sits on plain background, so the moving part and the fixed part read apart; the Switch cell now takes the composer's own ground instead, so the cell and the composer row under the belt read as one surface. The belt draws no chevron; its tint and its fade carry the scroll cue by themselves, and the fixed Switch pill's tap area now ends at its own hairline instead of reaching over the scroller. The belt's right edge now carries exactly one fade, the Switch block's own, constant at every scroll position, and the scroller's padding is measured off that block so the last pill always scrolls fully into view instead of hiding under it.
...
- **A bare Switch mark at the end of the actions belt opens the pane switcher.** The belt's right end carries the layers glyph above Send, behind a hairline and with no word or border of its own, a drag up anywhere on the belt still opens the switcher, and the 30px handle band above the composer is gone.
...
- **The machine's name moves into the pane header, onto the path line.** On a crew the host name and the cache countdown ride at the end of the header's second line, after the working directory, which gives way first when the line is tight; the pane menu stands alone in the corner, the row keeps the height it always had, and the actions belt no longer carries the name. The place and the meta now stand on one baseline, so the second line reads as one row rather than two. That line now names the workspace alone, because the tab strip under it already names the open tab.
- **One name on every screen.** A pane is called the same thing on the dashboard, in the switcher, on its own header, on its pill and in a push: the name you gave it, else the name the agent gave its session, else the title the pane's program prints, else the word for the agent running there. The pane header no longer leads with the space and tab, and no screen falls back to a directory for a name.
- **The place is the second line, and only the second line.** `space › tab` sits under the name on the dashboard row, in the switcher row and in the pane header, where the machine's name and the cache countdown still close the line; a push says the same place in its body. A card already grouped under its tab keeps the directory there instead, since its place is the heading above it.
...
- **Rows keep their order.** The bridge sends every pane in the multiplexer's own arrangement, space then tab then the pane's place in that tab, and the dashboard, the switcher and the pane strip keep it rather than re-sorting by activity or by pane id. A row moves when it changes section, not while you reach for it, and the dashboard's list of spaces now runs in the same order as the space strip.
- **The dashboard's rows sit under the workspace they live in.** What needs you stays on top, by urgency, listed once and never again under its workspace group; every other pane now sits under a heading that names its workspace and counts what is inside it, in the multiplexer's own machine and workspace order. Each row carries its tab on a second line, or `tab 2` when the tab has only a number, and the name sits in the middle of the row when the tab has neither, and every row of a group is the same height. Bare shells join their tab's group after its agents. The Working and Recent headings are gone with them, and so are Recent's fold and its sort toggle.
- **The tab row and the pane row under the header get compact.** Tab row 32px, pane pills 24px, text 11px, the size of the header's path line; every pill still answers a 44px tap. The tab row now draws no horizontal rule of its own: tabs are separated by vertical hairlines and the open tab is marked by its ground. The open tab is now an outlined pill instead of a stray hairline on its neighbour, the row has no hairlines between tabs at all, and nothing sits under the tab row any more — the pane row beneath it shares the tab row's own ground instead of falling through to the page's ambient black.
...
- **`hooks install` writes through a symlinked `~/.claude`, and still refuses a symlinked settings file.** Stow, chezmoi and similar tools point the whole config directory elsewhere; the write now follows that link instead of refusing it, while a settings file that is itself a symlink (home-manager's read-only profile) is still refused, with a worded error rather than a raw write failure for a dangling or unwritable link. Thanks @cryptiklemur (#190).
