...
- **The actions row is a belt.** One full-bleed band above the input, flush under the mirror, carries Collie's controls on its ground and the harness's commands in a section tinted with the harness's colour, so the row reads as one strip with two parts rather than two floating capsules. The scrolling part of the belt now takes a faint brand tint and the fixed Switch cell sits on plain background, so the moving part and the fixed part read apart; the Switch cell now takes the composer's own ground instead, so the cell and the composer row under the belt read as one surface. The belt draws no chevron; its tint and its fade carry the scroll cue by themselves, and the fixed Switch pill's tap area now ends at its own hairline instead of reaching over the scroller.
