...
| `removeWorktree` | `POST /api/workspace/:id/worktree/remove` | `worktree.remove {workspace_id, force}` → `{path, forced}`, closing the space with it (**H8**). Addressed by WORKSPACE and by nothing else: there is no path-addressed removal, so a checkout nothing shows cannot be removed at all. `force:false` on a dirty checkout answers `dirty_worktree_requires_force` with Git's own sentence | **no** (**T**) | **no** (**Z**) |
