...
| `fitToPhone` | `POST /api/pane/:id/fit` (take or renew, body `{"cols": …, "rows": …}`) and `DELETE /api/pane/:id/fit` (release) — the pane view's "Fit to phone" tap | a lease: one `terminal session control <terminal_id> --cols C --rows R` child held by the bridge, frames drained and never parsed, ended on release, on a two-minute lapse or on shutdown. Probed 2026-09-24 on herdr 0.9.1 (**API** § `terminal session control`): the PTY takes the leased size with or without a desktop client, and Herdr itself restores the desktop's rect on release. Another controller already holding the terminal answers `busy`; `--takeover` is never passed ([ADR 0049](./.adr/0049-fit-to-phone-is-a-leased-geometry-write.md)) | **no** (**T**) — unprobed | **no** (**Z**) — unprobed |
