...
import { NO_RUNTIME_FACTS, type CrewRuntimeMarker } from "../bridge/crew/staleness.ts";
import { adoptLeadership, rosterRowsOf } from "../bridge/crew/takeover.ts";
import type { StoredWarrant, TrustStoreData } from "../bridge/crew/trust-store.ts";
...
// What `collie crew status` SAYS about the deputy. Pure data in, `TonedLine[]` out — so the whole
// render matrix is pinned here rather than discovered on a live crew at 23:00.
//
...
function marker(over: Partial<CrewRuntimeMarker> = {}): CrewRuntimeMarker {
  return {
    ...NO_RUNTIME_FACTS,
    bootedAt: T0,
    pid: 1,
