...
// boundary `cli/crew-update.test.ts` draws, for the sharper version of its reason — this verb
...
interface HarnessOptions {
   ...
   * same key and one stamp lands second (`bridge/crew/router.ts` → `MEMBERSHIP_PATHS`), so it is a
  ...
  /** Per-member `hello` answers for `crew status`: the warrant generation, or `false` for silence. */
  ...
  /** Seeded files, for the runtime marker `crew status` reads. */
  files?: SeededFiles;
  /**
...
function marker(
  ...
  facts: Partial<CrewRuntimeFacts> = {},
...
describe("crew deputy refuses before it mints", () => {
  test("a solo collie has no crown to deputise for", async () => {
    ...
    expect(await cmdCrewDeputy(h.deps, ["nas"])).toBe(EXIT.STATE);
  ...
  test("a lead cannot deputise itself", async () => {
    const h = harness();
    expect(await cmdCrewDeputy(h.deps, ["desk"])).toBe(EXIT.STATE);
  ...
  test("a member this lead does not pin is a typo, not a consent", async () => {
    const h = harness();
    expect(await cmdCrewDeputy(h.deps, ["ghost"])).toBe(EXIT.STATE);
  ...
  test("a bare `crew deputy` prints usage and this lead's peers, and mints nothing", async () => {
    const h = harness();
    expect(await cmdCrewDeputy(h.deps, [])).toBe(EXIT.USAGE);
...
describe("crew deputy arms the whole crew under one consent", () => {
  test("mints, restarts this lead, pushes to every peer, then restarts them all", async () => {
    const h = harness();
    expect(await cmdCrewDeputy(h.deps, ["nas"])).toBe(EXIT.OK);
  ...
  test("ONE confirmation covers the batch, and it names every machine", async () => {
    const h = harness();
    await cmdCrewDeputy(h.deps, ["nas"]);
  ...
  test("a non-interactive run aborts legibly and never reads EOF as yes", async () => {
    ...
    expect(await cmdCrewDeputy(h.deps, ["nas"])).toBe(EXIT.FAIL);
  ...
  test("the armed generation is remembered per member, beside the ssh route and never in the store", async () => {
    const h = harness();
    await cmdCrewDeputy(h.deps, ["nas"]);
    const recorded = parseCrewOps(h.ops.contents()!)?.members;
  ...
  test("naming a second deputy supersedes the first in one step, at the next generation", async () => {
    ...
    expect(await cmdCrewDeputy(h.deps, ["attic"])).toBe(EXIT.OK);
...
describe("a peer this operator cannot ssh into is REPORTED", () => {
  test("it gets the warrant, is named in the exact `anchor INACTIVE` shape, and is not restarted", async () => {
    ...
    expect(await cmdCrewDeputy(h.deps, ["nas"])).toBe(EXIT.OK);
    // It still received the warrant — phase 1 rides the crew link and needs no ssh at all.
    expect(h.pushed.map((p) => p.member).toSorted()).toEqual(["attic", "nas"]);
...
describe("a push the verb cannot confirm never becomes a recorded anchor", () => {
  ...
  test("the drill's exact sequence: 401 that never clears → member failed, NOT restarted, no ops write, non-zero exit", async () => {
    ...
    expect(parseCrewOps(h.ops.contents()!)?.members.attic?.anchoredGeneration ?? null).toBeNull();
    // The member that DID store it is unaffected — one failure is not a cancelled run.
    expect(parseCrewOps(h.ops.contents()!)?.members.nas?.anchoredGeneration).toBe(1);
  });
  ...
  test("the sweep winning the race is not a failure — the peer's own report is the confirmation", async () => {
    ...
    expect(parseCrewOps(h.ops.contents()!)?.members.attic?.anchoredGeneration).toBe(1);
  });
...
describe("a re-run re-syncs rather than minting", () => {
  ...
  test("a re-run after fixing the ssh record completes the anchoring the first run could not", async () => {
    ...
    await cmdCrewDeputy(first.deps, ["nas"]);
    const armed = parseCrewOps(first.ops.contents()!)!.members;
    ...
    expect(await cmdCrewDeputy(second.deps, ["nas"])).toBe(EXIT.OK);
    ...
    expect(parseCrewOps(second.ops.contents()!)?.members.attic?.anchoredGeneration).toBe(1);
  });
  ...
  // to restart a crew that was already fully armed. It asks the machines now.
  test("the drill's sequence: warrant stored, restarted OUT OF BAND — the re-run asks nothing and exits 0", async () => {
    ...
    const members = parseCrewOps(h.ops.contents()!)!.members;
...
describe("crew deputy --revoke", () => {
  test("mints a generation naming NOBODY and pushes it — an absence would prove nothing", async () => {
    const h = harness({ store: withWarrant(twoPeers(), "nas") });
    expect(await cmdCrewDeputy(h.deps, ["--revoke"])).toBe(EXIT.OK);
    expect(h.pushed).toHaveLength(2);
    for (const push of h.pushed) {
  ...
  test("offers the same restart batch — a stored revocation is inert at the listener too", async () => {
    const h = harness({ store: withWarrant(twoPeers(), "nas") });
    await cmdCrewDeputy(h.deps, ["--revoke"]);
    expect(h.confirms[0]).toBe("restart collie on nas, attic to retire the old deputy's anchor? [y/N]");
    expect(legs(h)).toEqual(["nas.example:probe", "attic.example:probe", "nas.example:restart", "attic.example:restart"]);
  });

  // The incident's second half. `crew remove <deputy>` left the designation naming the removed
  // machine, so `crew status` printed a deputy that `--revoke` then refused to find. Both surfaces
  ...
  test("after `crew remove <deputy>` the two surfaces agree that nobody is named", async () => {
    ...
    // `crew status`' designation line is empty, ...
...
// ── `crew status`, the lead's view (RFC §10, §5) ─────────────────────────────
...
describe("crew status on the lead", () => {
  ...
  // ── THE LIVE DRILL, BUG 5 (§18.17) ────────────────────────────────────────
  // The lead told the operator to restart a deputy whose own `crew status` read `deputy role ACTIVE
  ...
  test("a member with no ops record gains none — an anchor with no ssh route invents a field", async () => {
    ...
    await cmdCrewStatus(h.deps, []);
    expect(parseCrewOps(h.ops.contents() ?? "{}")?.members.nas).toBeUndefined();
  });
...
// ── `crew status` on a machine that was deposed (RFC §8.2, §8.3) ─────────────
...
describe("crew status on a deposed machine", () => {
  ...
  test("stranded by a rotation is named as that, never as a self-heal that went wrong", async () => {
    ...
    await cmdCrewStatus(h.deps, ["--no-probe"]);
    expect(text(h.io)).toContain("the crew secret was rotated while it was away");
  });
...
// ── RFC §6.4 and §16 decision 4: what `crew deputy` will not do, and what it warns about ──
...
describe("crew deputy says the same-origin prerequisite once (RFC §16, decision 4)", () => {
  test("with no shared origin configured it WARNS, names what is lost, and refuses nothing", async () => {
    ...
    // The fallback is named rather than implied: this crew keeps everything but the phone-first half.
...
// ── `crew status` on the deputy: its own door (RFC §6.2, §6.3, §10.1) ────────
...
// ── `crew status` on the lead: the refused pairing sync (RFC §6.5, §18.14) ───
...
// ── `crew status` on a NEW lead: the members it has not told (RFC §7.1, §9) ──
...
describe("crew status renders the pending re-pins a takeover left behind", () => {
  ...
  test("a member that WAS told carries no row, and neither does one from before the field", async () => {
    ...
    await cmdCrewStatus(g.deps, ["--no-probe"]);
