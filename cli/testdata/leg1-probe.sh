...
say envport "$ENVPORT"
CHECKOUT=""
for _d in "$HOME/.collie" "$HOME/collie" "$HOME/.local/share/collie/current" "$HOME"/.config/herdr/plugins/github/*/ "$HOME"/.config/herdr/plugins/local/*/; do
  _d=${_d%/}
...
if [ -n "$LISTEN" ]; then
  if printf '%s\n' "$LISTEN" | grep -q "[:.]8787[[:space:]]"; then PORTSTATE=busy; else PORTSTATE=free; fi
fi
say port "$PORTSTATE"
say probe ok
