import { deposedOutcomeLines } from "../bridge/crew/deposed.ts";
...
import {
  ...
  warrantNamesSelf,
  type StandbyFacts,
} from "../bridge/crew/standby.ts";
import type { StandbyDevices } from "../bridge/crew/standby-devices.ts";
import { checkpointStale, type CrewRuntimeMarker } from "../bridge/crew/staleness.ts";
import type { TrustedMember, TrustStoreData, Warrant } from "../bridge/crew/trust-store.ts";
import { currentWarrant, verifyWarrantSignature, warrantExpired, warrantExpiresAt } from "../bridge/crew/warrant.ts";
import { herdrActionCommand, type Environment } from "./context.ts";
import type { Tone, TonedLine } from "./render.ts";

// What `collie crew status` says about the deputy — on the lead that named one, on the peer that
// holds the warrant, and on the machine that was deposed (RFC §10, §8.3, §5).
//
// ── EVERY FUNCTION HERE IS PURE ──────────────────────────────────────────────
// Data in, `TonedLine[]` out. `cmdCrewStatus` does the probing and the emitting; this module does
...
// one word for both would be reporting a crew as armed that is not. Each side knows a different half:
//
...
// `crew status` does not print is a door nobody can explain.
 ...
 * discovering months later that their idle crew arms its own standby door every night. The `30_000`
 ...
 *
 * The state lives in the running process (`bridge/crew/deposed.ts`) and reaches this verb through
 * the runtime marker, so a `crew status` on a machine whose bridge is down prints nothing here — and
 ...
 */
export function deposedLines(marker: CrewRuntimeMarker | null, instance: string | null): TonedLine[] {
  ...
  return [
    line("", "plain"),
    line(`⚠ DEPOSED — this machine led crew ${crew} until ${iso(state.at)}.`, "bad"),
 ...
 *
 * A lead with peers and no warrant is a crew nobody may take over, and RFC §8.3 requires that to be
 ...
 * round to it" will not realise the crew has been without a deputy since the outage. A revocation and
 * a crew that never named one are the two innocent cases and are spelled as such.
 */
 ...
 * read that sentence on a deputy whose own `crew status` said `deputy role ACTIVE at this boot`.
...
export function leadContactLines(
  ...
  // gone" and "my lead is calling and I am no longer in the crew", and only this collie can tell.
 ...
 * notice. It costs one ECDSA verification per `crew status`.
 */
...
function contactFacts(marker: CrewRuntimeMarker) {
  return {
 ...
 * the sentence. Nothing here re-derives any of them, because a `crew status` that computed its own
 ...
 * heading about one would be four lines of nothing on every `crew status` in the crew.
 */
 ...
 */
export function pairingCollisionLines(marker: CrewRuntimeMarker | null, now: number): TonedLine[] {
