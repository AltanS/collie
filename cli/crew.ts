...
import { signDial, signRequest } from "../bridge/crew/signing.ts";
import { dialTls } from "../bridge/crew/transport.ts";
import { deriveMode } from "../bridge/crew/mode.ts";
import { crewOpsPath, CrewOpsStore, type OpsRecord } from "../bridge/crew/ops-store.ts";
import {
  crewHelloBudget,
  crewTimeoutBudget,
  ...
  type CrewLink,
  type PeerOutcome,
} from "../bridge/crew/peer-client.ts";
...
import { currentWarrant } from "../bridge/crew/warrant.ts";
import {
  parseStandbyDevices,
  standbyDevicesPath,
  type StandbyDevices,
} from "../bridge/crew/standby-devices.ts";
import { crewRuntimePath, parseMarker, rosterDrift } from "../bridge/crew/staleness.ts";
...
import {
  ...
  peerWarrantLines,
  standbyDoorLines,
} from "./crew-status-deputy.ts";
import type { Tone, TonedLine, Ui } from "./render.ts";
// Type-only, so it is erased: the runtime edge to `cli/remote.ts` is the dynamic import in `cmdCrew`.
import type { CrewAddDeps } from "./remote.ts";
import type { Exec, Files } from "./sys.ts";
import { tailnetName } from "./tailnet.ts";

// The crew verbs: `crew invite`, `join`, `leave`, `crew status`, `crew rotate`, `crew remove`,
// `promote`, `reconnect` — the ONLY way a machine enters or leaves a crew (M4/07).
//
// ── WHAT LIVES HERE AND WHAT DOES NOT ────────────────────────────────────────
// Nothing in this file decides what a trust store should contain. Every mutation is one of the pure
// transitions in `bridge/crew/enrollment.ts`, committed through `commitCrewChange`, so the engine's
...
// literal form warns on stderr. The crew secret is never an argument, never printed and never
...
// `TrustStore` reads its file once per process (bridge/crew/trust-store.ts) and the running bridge
...
/** Where the verbs reach the world. Every field is a seam `cli/crew.test.ts` supplies a fake for. */
export interface CrewDeps {
  ...
  /**
   * How the operator reached each member over SSH — written by `crew add`, read by `crew update`,
   * dropped by `crew remove`. Operator-local convenience beside the trust store, never trust and
   * never a wire field (ADR 0016), which is why it is a second store and not a column in the first.
   */
  readonly ops: CrewOpsStore;
   ...
   *
   * The optional `io` is where the nested verb's own output goes. Only `crew add` passes it, and
  ...
  /**
   * The free-text ask, behind the seam `crew add` already owns (`CrewAddDeps.prompt` narrows this
 ...
 */
export type ProbeDeps = Pick<CrewDeps, "ctx" | "fetch" | "now">;

/** The seams the staleness comparison needs. Same reason as {@link ProbeDeps}. */
export type DriftDeps = Pick<CrewDeps, "ctx" | "io" | "files">;
...
// operator-owned ingress — reverse proxies, tunnels, WAN — that a same-crew member dial never does,
...
/** The crew timeout budget for a one-shot verb: the default, clamped by the poll interval as usual. */
function timeoutFor(ctx: CliContext): number {
  return crewTimeoutBudget(pollFor(ctx), ctx.env);
}
 ...
 * budget `crew status` therefore printed `unreachable` for a healthy member categorically, no matter
...
function patientTimeoutFor(ctx: CliContext): number {
  return crewHelloBudget(pollFor(ctx), ctx.env);
}
...
export function clientFor(deps: ProbeDeps, data: TrustStoreData, secret: string): PeerClient {
  return new PeerClient({
    ...
    //
    // **Why the lead is the exception, and why the test is its ROLE.** `bridge/crew/transport.ts`'s
    // design note states the law this obeys: "A LEAD does not pin its listener at all. Its crew
    ...
    // layer, and `sign`/`dialSign` below run on EVERY call this client makes, carrying the crew
    ...
    // peer→lead direction, where the transport cannot pin (`bridge/crew/transport.ts`); signing the
    // whole set means `crew status` and `reconnect` can probe a lead at all, and it costs one ECDSA
    ...
    // A verb that dialled such a peer without one — `crew rotate`, `crew status` — would read the
    // whole crew as unauthorized.
...
export const linkOf = (member: TrustedMember): CrewLink => ({
 ...
 */
function publicFrontDoor(deps: CrewDeps): string | null {
 ...
 */
export function joinHost(deps: CrewDeps): string | null {
  const configured = publicFrontDoor(deps);
  if (configured !== null) {
...
/** The parsed flag set every crew verb shares: `--flag value` pairs plus bare positional arguments. */
 ...
 */
export function parseCrewArgs(args: readonly string[], bareFlags: readonly string[] = ["force"]): CrewArgs {
...
export async function readToken(
  raw: string,
  deps: Pick<CrewDeps, "files" | "io" | "readStdin">,
): Promise<string | null> {
 ...
 *
 * Exported for `crew add` (cli/remote.ts), which mints its invite through the same path — and must
 * do so through THIS function rather than a second one, or "solo mints nothing" stops being
 * structural.
 */
export async function ensureStore(deps: CrewDeps, label: string | undefined): Promise<TrustStoreData | null> {
...
async function applyLocally(deps: CrewDeps, what: string): Promise<void> {
 ...
 */
async function clearOwnHerdTags(deps: CrewDeps): Promise<void> {
...
export async function cmdCrewInvite(deps: CrewDeps, args: readonly string[]): Promise<number> {
  ...
  // The host the joiner types after `collie crew join` — this lead's, and `--address` still wins.
 ...
 */
async function resolveToken(deps: CrewDeps, address: string, given: string | undefined): Promise<string | null> {
  if (given !== undefined) {
 ...
 */
function joinLabel(deps: CrewDeps, flag: string | undefined): string | undefined {
...
async function postEnrollment(deps: CrewDeps, origin: URL, body: string): Promise<EnrollAttempt> {
 ...
 */
async function consentToPlaintext(deps: CrewDeps, plain: URL): Promise<boolean> {
...
/**
 * Refuse a plaintext hop. Over `http://` both the invite token and the crew secret cross the wire in
 ...
 */
function refusePlaintext(deps: CrewDeps): void {
...
/**
 * `collie crew join <lead-address> [<token>]` — §8.2, run on the peer, once.
 *
 * Distinct outcomes get distinct exit codes (spec requirement), because "it didn't work" is the one
 * answer an operator cannot act on: already in a crew is `3`, a refused token is `4`, an address that
 * did not answer is `5`.
 */
export async function cmdJoin(deps: CrewDeps, args: readonly string[]): Promise<number> {
  ...
  // A lead whose crew surface is plain HTTP on the port we assumed `https://` for fails the
  ...
  const accepted = await commitCrewChange(deps.store, deps.audit, (current) =>
    current === null ? null : acceptEnrollment(current, parsed, dialed, deps.now()),
  );
  if (accepted === null) return EXIT.FAIL;

  deps.io.out(`✓ joined crew "${parsed.crewName}" as "${accepted.memberId}"`);
 ...
 * **port 8787** — the port a collie's own listener binds, and therefore the only thing `collie crew
...
export function enrollUrl(address: string): string | null {
  ...
  return base === null ? null : new URL(CREW_ENROLL_PATH, base).toString();
}

// ── leave (on the peer) ──────────────────────────────────────────────────────

/**
 * `collie leave` — drop the roster entry, the pinned material and the crew secret (§8.4).
 *
...
export async function cmdLeave(deps: CrewDeps): Promise<number> {
  ...
  const left = await commitCrewChange(deps.store, deps.audit, (current) =>
    current === null ? null : leaveCrew(current),
  );
  if (left === null) return EXIT.FAIL;

  // ── THE OLD CREW'S DEPUTY STATE GOES WITH IT ────────────────────────────────
  // `leaveCrew` clears the fields inside the trust store; `standby-devices.json` is its own file
  // (`bridge/crew/standby-devices.ts`) and had to be forgotten here. It is the OLD lead's paired
  // phones — bearer-token hashes for a crew this machine has just left — and keeping it would leave
  ...
  for (const line of retireCrewBind(deps)) deps.io.out(line);
 ...
 *
 * `crew add`'s configure leg writes `COLLIE_HOST=<the address the lead dials>` — a wide bind — and
 ...
 * The other candidate fix was for `crew add` to write `COLLIE_ALLOW_NON_LOOPBACK_BIND=1` beside the
 * wide bind it chose. That is the substitution ADR 0013 exists to refuse. A peer's off-loopback
 * listener is admitted BY CONSTRUCTION — two independent factors, pinned mutual TLS plus the crew
 ...
 * exemption past them would leave a machine with no crew, a wide bind, its own front door and its
 * browser write gates back on — reachability standing in for authorisation, which is the one
 * substitution the whole posture is built to refuse. The exemption is the crew's; it lapses with it.
 *
 ...
 */
function retireCrewBind(deps: CrewDeps): string[] {
...
export async function cmdCrewStatus(deps: CrewDeps, args: readonly string[]): Promise<number> {
  ...
  // it; the bind never gates (pinned mTLS + the crew secret do, §3), it only bounds who can attempt.
  ...
  // …but first, whether this machine is still the lead it thinks it is. A machine rejoining a crew
  ...
  // roster rather than beside that member's row: it is a fact about the CREW's readiness, and it is
  ...
  for (const m of members) {
    ...
    if (outcome.ok) {
      ...
      // bridge's own sweep could also stamp this later; today `crew status` is the clearer.
      if (m.contactedAt === null) {
        ...
        await commitCrewChange(deps.store, deps.audit, (current) =>
          current === null
            ? null
            : {
                next: {
  ...
  if (deps.ui != null) await deps.ui.crewMembers(banked);
  return EXIT.OK;
}
 ...
 * this machine's own registry would arm its own write gate (`bridge/crew/standby-devices.ts`).
 */
function syncedDevicesOnDisk(deps: CrewDeps): StandbyDevices | null {
  const raw = deps.files.read(standbyDevicesPath(deps.ctx.stateDir));
  return raw === null ? null : parseStandbyDevices(raw);
}

/**
 * The one thing `crew status` knows that the store alone does not: **whether the running bridge is
 * serving this roster**.
 *
 ...
 * OFFLINE view (`crew status --no-probe`, and every member that is not answering right now) keeps
 * printing `anchor INACTIVE` about a crew that is armed. A probe that got the answer writes it down,
 * so the two views converge instead of disagreeing until the next `crew deputy`.
 *
 ...
 * `CrewOpsStore.record` returns a boolean at all: a convenience file is never worth failing a
 ...
 * without the other is silently clamped ({@link crewTimeoutClampWarning}).
 *
 ...
 * for `crew status` (§7.1), and erasing it here would leave the renderer casting a body the client
 * has already parsed.
 */
...
export interface MemberReach {
   ...
   * (`bridge/crew/router.ts`'s `SIGNABLE_PATHS`, RFC §8.6: `leave`, `lead`, `hello` and the two
   ...
   * the link. Asking it produced a red `STARVED` line, on a healthy crew, under a remedy — raise two
 ...
 * starved the phone: `crew status` printed `reachable`, `doctor` printed `member-reach ✓`, and every
...
// ── crew rotate (on the lead) ────────────────────────────────────────────────

/**
 * Reissue the crew secret and distribute it (§8.4).
 *
...
export async function cmdCrewRotate(deps: CrewDeps): Promise<number> {
  ...
  const dropped = await commitCrewChange(deps.store, deps.audit, (current) =>
...
// ── crew rename (on the lead) ────────────────────────────────────────────────

/** The longest crew name this verb accepts, in code points. Long enough for a sentence, short
 * enough to sit on one line of the phone's crew page beside the crew id. */
 ...
 * crew id: it travels once, in the lead's enrollment answer (bridge/crew/enrollment.ts §8.2), and a
 * member stores it and never shows it. Admission, signing and every gate read the crew id and the
 ...
 */
export async function cmdCrewRename(deps: CrewDeps, args: readonly string[]): Promise<number> {
  ...
  // The transition is written here rather than in `bridge/crew/enrollment.ts` on purpose: that file
  // is on the wire-shape guard's list (scripts/check-crew-wire.sh) because a change there can move
...
// ── crew remove (on the lead) ────────────────────────────────────────────────

/** `collie crew remove <member>` — unpin and forget (§8.4). Local, and deliberately not a request. */
export async function cmdCrewRemove(deps: CrewDeps, args: readonly string[]): Promise<number> {
  const { positional } = parseCrewArgs(args);
  ...
  const removed = await commitCrewChange(deps.store, deps.audit, (current) =>
...
/**
 * The half of `crew remove` that helps: the command the operator now has to run on the OTHER side.
 *
 * `crew remove` is correct and says so plainly — the removed machine keeps its copy of the crew until
 ...
 * feared a non-member lingering in `crew update`'s target list; that fear does not survive reading
 * the code — `crew update` builds its targets from the ROSTER and only then looks a member up here
 * (`cli/crew-update.ts`), so a row for a machine that is not a member can never be dialled by it.
 ...
 */
function leaveTheOtherSideLines(deps: CrewDeps, record: OpsRecord | null): string[] {
  const where = crewOpsPath(deps.ctx.stateDir);
  if (record === null) {
    return [
...
// ── crew set-address (on the lead) ───────────────────────────────────────────

/**
 * Why this address is not one a crew link may be dialled at — one line, or `null` when it is fine.
 *
 ...
 *    (see {@link selfAddress}'s `crew-listener` note, which is the same trap on the minting side).
 */
export function crewAddressRefusal(address: string): string | null {
...
/**
 * The remedy `crew status` offers a peer whose LEAD did not answer — the other half of
 * {@link schemedAddressLines}, which is deliberately silent on this row.
 *
 * There is exactly one verb here, and it is not `crew set-address`. A member's address is corrected
 ...
 * this machine ({@link cmdCrewSetAddress}). Naming the wrong one sent an operator to a command that
 ...
 * crew really does want a bare `host:port` for its own pinned dial ({@link crewAddressRefusal}).
 * The verb named there is the lead's, `crew set-address`, because the lead is the machine that
 * dials that member. On the other side of the link the verb is `collie reconnect <address>`, run
 * on the machine being re-pointed — which is what {@link cmdCrewSetAddress} says when a peer
 ...
 * *correction* of a member's address and refuses a value the crew could not dial. A wrong address is
 * a hint, never an identity (§4) — the pin is untouched either way, so the cost of a bad one is
 * exactly one more run of this verb.
 */
export async function cmdCrewSetAddress(deps: CrewDeps, args: readonly string[]): Promise<number> {
  const { positional } = parseCrewArgs(args);
  ...
  const moved = await commitCrewChange(deps.store, deps.audit, (current) =>
...
// ── crew approve-promote (on the lead) ───────────────────────────────────────

/**
 * `collie crew approve-promote <member-id>` — the operator's consent, on the machine being taken
...
export async function cmdCrewApprovePromote(deps: CrewDeps, args: readonly string[]): Promise<number> {
  ...
  if (cancelling) {
    const cancelled = await commitCrewChange(deps.store, deps.audit, (current) =>
  ...
  const approved = await commitCrewChange(deps.store, deps.audit, (current) =>
...
export async function cmdPromote(deps: CrewDeps, args: readonly string[]): Promise<number> {
  ...
  // This machine is taking the crown, so what it advertises — to the crew AND to the phone below —
  ...
  if (handover.ok) {
    ...
    // The demoted lead is a member of this crew like any other, and it just told us its own pin is
  ...
  const promoted = await commitCrewChange(deps.store, deps.audit, (current) =>
  ...
  // the sole anchor of its listener (`peerListenerTls`, bridge/crew/transport.ts), so a dial from a
...
export async function cmdReconnect(deps: CrewDeps, args: readonly string[]): Promise<number> {
  ...
  if (outcome.ok) {
    // The same second question `crew status` asks, for the same reason: an address that answers the
...
// ── `collie crew <sub>` dispatch ─────────────────────────────────────────────

/** The `crew` sub-verbs, in the order the help prints them. */
export const CREW_SUBCOMMANDS = [
  "invite",
  // `join` and `leave` are the CANONICAL spellings — every other crew verb lives under `crew`, and
...
export function crewUsage(): string {
  return `usage: collie crew {${CREW_SUBCOMMANDS.join("|")}}`;
}
 ...
 * off so every flag grammar stays in the verb (`cli/program.ts`), which used to make `collie crew
 * --help` — the most ordinary thing anyone types — answer `error: unknown crew subcommand`. They
 * print exactly what `collie crew` and `collie crew help` print, and share their exit code: this is a
 * spelling, not a second surface.
 */
const CREW_HELP_SPELLINGS: ReadonlySet<string> = new Set(["help", "--help", "-h"]);

export async function cmdCrew(deps: CrewAddDeps, args: readonly string[]): Promise<number> {
  const [sub, ...rest] = args;
  switch (sub) {
    case "invite":
      return cmdCrewInvite(deps, rest);
    case "join":
    ...
    // in the switch is local, and `crew add` is the one verb that reaches another machine.
    case "add": {
      const { cmdCrewAdd } = await import("./remote.ts");
      return cmdCrewAdd(deps, rest);
    }
    // Same reason `add` is imported at call time: `cli/crew-update.ts` reaches back into this module
    // for `parseCrewArgs` and `probeMembers`, and a static import here would close a cycle.
    case "update": {
      const { cmdCrewUpdate } = await import("./crew-update.ts");
      return cmdCrewUpdate(deps, rest);
    }
    // Same reason `add` and `update` are imported at call time: `cli/crew-deputy.ts` reaches back into
    // this module for `parseCrewArgs`, `clientFor` and `linkOf`, and a static import here would close
    // a cycle.
    case "deputy": {
      const { cmdCrewDeputy } = await import("./crew-deputy.ts");
      return cmdCrewDeputy(deps, rest);
    }
    case "status":
      return cmdCrewStatus(deps, rest);
    case "rotate":
      return cmdCrewRotate(deps);
    case "rename":
      return cmdCrewRename(deps, rest);
    case "remove":
      return cmdCrewRemove(deps, rest);
    case "set-address":
      return cmdCrewSetAddress(deps, rest);
    case "approve-promote":
      return cmdCrewApprovePromote(deps, rest);
    default:
      if (sub !== undefined && sub !== "" && !CREW_HELP_SPELLINGS.has(sub)) {
        deps.io.err(`error: unknown crew subcommand \`${sub}\``);
      }
      deps.io.err(crewUsage());
 ...
 * a crew verb touches — the store, the transport, the clock, entropy, the identity minter — is named
 * in one place, and `cli/crew.test.ts` replaces exactly this object.
 */
export function crewDeps(
  base: {
    ctx: CliContext;
    io: Io;
    ui?: Ui | null;
  ...
  return {
    ...
    ops: new CrewOpsStore(base.ctx.stateDir),
    audit,
    fetch: (url, init) => fetch(url, init),
    now: () => Date.now(),
    random: randomToken,
    // Built per call, NOT eagerly: `tailnetName` shells out to `tailscale`, and `crewDeps` is
    // constructed for every crew verb — including the ones that never mint. An eager build would
    // make `collie crew status` run a tailnet lookup it has no use for.
    //
    // The CN and SANs are legibility, not trust: a pin is a fingerprint and the dialling side
    // overrides the name check (`bridge/crew/transport.ts`), so a member that roams stays the same
    ...
    // Bun's built-ins behind a tty check, exactly as `crew add` and `stt setup` guard theirs: a
    // question nobody can answer must refuse legibly rather than read EOF as an answer. `crewAddDeps`
...
/** The audit log a crew verb writes through — the same file, same mode, the bridge appends to. */
export async function crewAudit(ctx: CliContext): Promise<AuditLog> {
