...
import { fp, leadStore, material, member, CREW, peerStore, T0 } from "../bridge/crew/fixtures.ts";
import type { CrewTlsOptions } from "../bridge/crew/transport.ts";
import {
  serializeTrustStore,
  TrustStore,
  type TrustStoreData,
 ...
 * produces.
 */
function harness(initial: TrustStoreData | null, replies: Reply[] = [], over: Partial<CrewAddDeps> = {}): Harness {
  ...
  const deps: CrewAddDeps = {
    ...
    fetch: async (url, init) => {
      ...
      requests.push({
        ...
        headers,
        // Every crew verb sends a `JSON.stringify` string; anything else is recorded as its text so
    ...
    // `crew add`'s own seams, present so `cmdCrew` can be dispatched here and REFUSING, so nothing
...
/** A crew response: 200, with the two headers §6 requires so `PeerClient` accepts it. */
...
describe("parseCrewArgs", () => {
  test("splits positionals, `--flag value`, `--flag=value` and bare flags", () => {
    const parsed = parseCrewArgs(["desk.ts.net", "-", "--label", "laptop", "--address=nas:1", "--force"]);
  ...
  test("a value-taking flag with nothing after it is empty, not the next flag", () => {
    expect(parseCrewArgs(["--label", "--force"]).flags).toEqual({ label: "" });
  });
});
...
describe("selfAddress — the port is explicit exactly where the dial needs it", () => {
  ...
  test("the appended port is the instance's own, not the default", () => {
    ...
    expect(selfAddress(h.deps, undefined, "crew-listener")).toBe("laptop.tail.ts.net:9000");
  });
  ...
  test("the operator's --address is taken VERBATIM — reachability is theirs to own (§8.2)", () => {
    ...
    for (const given of [
      ...
      expect(selfAddress(h.deps, given, "crew-listener")).toBe(given);
  ...
  test("an empty --address is no address at all, and falls back to the derived one", () => {
    // `parseCrewArgs` gives a value-taking flag with nothing after it the empty string.
    const h = harness(null);
    expect(selfAddress(h.deps, "", "crew-listener")).toBe("laptop.tail.ts.net:8787");
  });
  ...
  test("no tailnet name is null for both kinds — never a localhost the far side would dial", () => {
    ...
    expect(selfAddress(h.deps, undefined, "crew-listener")).toBeNull();
...
describe("collie crew invite", () => {
  ...
  test("it materialises the store — and identity minting refusing is the whole verb failing", async () => {
    ...
    expect(await cmdCrewInvite(refused.deps, [])).toBe(EXIT.FAIL);
  ...
  test("a peer refuses: invites are minted on the lead", async () => {
    ...
    expect(await cmdCrewInvite(h.deps, [])).toBe(EXIT.STATE);
...
describe("collie join", () => {
  ...
  test("enrolls, pins the lead, and never puts a credential in argv or a URL", async () => {
    ...
    expect(JSON.parse(req.body)).toEqual({
      ...
      // The joiner is becoming a PEER, so what it hands the lead is its own crew listener — host AND
...
// `crew join` and `crew leave` are canonical; `collie join` / `collie leave` are aliases onto the
...
describe("`collie crew join|leave` and their top-level aliases", () => {
  test("`crew join` runs `cmdJoin` — same requests, same words", async () => {
    const viaCrew = harness(null, [jsonReply(ENROLLED, 200, "desk")]);
    expect(await cmdCrew(viaCrew.deps, ["join", "desk.ts.net", "-"])).toBe(EXIT.OK);
    const direct = harness(null, [jsonReply(ENROLLED, 200, "desk")]);
    expect(await cmdJoin(direct.deps, ["desk.ts.net", "-"])).toBe(EXIT.OK);
    expect(viaCrew.requests.map((r) => r.url)).toEqual(direct.requests.map((r) => r.url));
    expect(text(viaCrew.io)).toBe(text(direct.io));
  });

  test("`crew leave` runs `cmdLeave` — same exit code, same words", async () => {
    const viaCrew = harness(null);
    expect(await cmdCrew(viaCrew.deps, ["leave"])).toBe(EXIT.STATE);
    const direct = harness(null);
    expect(await cmdLeave(direct.deps)).toBe(EXIT.STATE);
    expect(text(viaCrew.io)).toBe(text(direct.io));
    expect(text(viaCrew.io)).toContain("not in a crew");
  });

  test("the `crew` usage block names both of them", async () => {
    const h = harness(null);
    expect(await cmdCrew(h.deps, [])).toBe(EXIT.USAGE);
...
// what is pinned, by identity through `aliasOf` and per sub-verb over `CREW_SUBCOMMANDS`: a twelfth
// sub-verb cannot reach one spelling and miss the other.

describe("`collie crew` and its `pack` alias", () => {
  const crew = findCommand("crew");
  // `pack`, `viaPack` and the notice name the ALIAS spelling itself, so they keep the old word.
  ...
  test("every sub-verb is ONE function under both spellings", () => {
    expect(crew?.subcommands?.map((c) => c.name)).toEqual([...CREW_SUBCOMMANDS]);
    expect(pack?.subcommands?.map((c) => c.name)).toEqual([...CREW_SUBCOMMANDS]);
    for (const name of CREW_SUBCOMMANDS) {
  ...
  const table = (): readonly Command[] => {
    ...
    return [
      { name: "crew", summary: "stand-in crew", subcommands: [status], run: bare },
      {
        ...
        internal: true,
        subcommands: [crewAliasRow(status)],
...
describe("an unreachable member is described in Collie's words, not Bun's", () => {
  // The exact string Bun throws, which reached `crew status`'s link line, the 503 body a phone
...
describe("collie leave", () => {
  ...
  // F12: `crew add` writes COLLIE_HOST=<the address the lead dials> — a wide bind. Peer mode
  ...
  // A peer left crew A as its armed deputy at warrant generation 3, kept `deputy`, `warrant` and
  // `standbyRoster`, and joined crew B. Crew B's brand-new lead read generation 3 on `hello`, parked
...
describe("collie crew status", () => {
  ...
  test("renders mode, members, pinning, secret pickup and reachability", async () => {
    ...
    // The effective crew-listener bind (COLLIE_HOST, resolved) is shown so the operator sees it. The
    ...
    expect(rendered).not.toContain(CREW.secret);
  ...
  // and `crew status` is the one place it is rendered. `incompatible` stays §7's protocol mismatch.
  ...
  // not re-read it. `crew status` is where an operator finds out, because nothing else can tell them.
...
describe("collie crew rotate", () => {
  ...
  test("rotates locally FIRST, then distributes with the SUPERSEDED secret", async () => {
    ...
    for (const req of h.requests) {
      ...
      expect(req.headers.authorization).toBe(`Bearer ${CREW.secret}`);
...
// ── crew rename ──────────────────────────────────────────────────────────────
// The name is display data keyed by the crew id, so the whole verb is one write to this lead's own
...
describe("collie crew rename", () => {
  ...
  test("an empty name, a too-long one and no name at all are usage errors that write nothing", async () => {
    ...
    expect(await cmdCrewRename(empty.deps, ["   "])).toBe(EXIT.USAGE);
    expect(text(empty.io)).toContain("cannot be empty");

    const long = harness(leadStore());
    expect(await cmdCrewRename(long.deps, ["n".repeat(65)])).toBe(EXIT.USAGE);
    ...
    expect(await cmdCrewRename(none.deps, [])).toBe(EXIT.USAGE);
  ...
  test("the usage block names it, and it is dispatchable under both spellings", async () => {
    ...
    expect(CREW_SUBCOMMANDS).toContain("rename");
  });
});
...
describe("collie crew remove", () => {
  ...
  test("an unknown member is a state error naming the verb that lists them", async () => {
    const h = harness(leadStore());
    expect(await cmdCrewRemove(h.deps, ["ghost"])).toBe(EXIT.STATE);
    expect(text(h.io)).toContain("collie crew status");
  });

  // The two surfaces used to disagree out loud: `crew status` printed `deputy nas — warrant
  // generation 1` on a lead whose `crew deputy --revoke` answered "this crew names no deputy".
  test("removing the DEPUTY drops the designation, and says so", async () => {
    ...
    // `crew status` reads the designation, so with it gone the two surfaces agree.
    expect(leadDeputyLines(h.data()!, T0)).toEqual([]);
    // The counter stays, so a later mint cannot re-issue a generation this crew has already used.
    expect(h.data()!.warrant?.warrant.generation).toBe(1);
  });

  test("no member id is a usage error", async () => {
    const h = harness(leadStore());
    expect(await cmdCrewRemove(h.deps, [])).toBe(EXIT.USAGE);
  });
});

// ── crew approve-promote ─────────────────────────────────────────────────────
...
describe("collie crew approve-promote — consent on the lead (§14.1)", () => {
  ...
  test("it arms a ten-minute consent, names the next step, and RESTARTS the bridge", async () => {
    ...
    expect(await cmdCrewApprovePromote(h.deps, ["nas"])).toBe(EXIT.OK);
  ...
  test("a peer has nothing to hand over, and a store with no crew has no handover at all", async () => {
    ...
    expect(await cmdCrewApprovePromote(onPeer.deps, ["desk"])).toBe(EXIT.STATE);
    expect(text(onPeer.io)).toContain("approved on the lead");
    const solo = harness(null);
    expect(await cmdCrewApprovePromote(solo.deps, ["nas"])).toBe(EXIT.STATE);
    expect(onPeer.restarts).toEqual([]);
  });

  test("a member id this lead does not pin is a typo, not a consent", async () => {
    const h = harness(leadWithNas());
    expect(await cmdCrewApprovePromote(h.deps, ["ghost"])).toBe(EXIT.STATE);
  ...
  test("no member id at all is a usage error, not an approval of nobody", async () => {
    const h = harness(leadWithNas());
    expect(await cmdCrewApprovePromote(h.deps, [])).toBe(EXIT.USAGE);
  });

  test("`--cancel` is BARE — it clears the approval and never swallows the next token", async () => {
    const h = harness(leadWithNas());
    await cmdCrewApprovePromote(h.deps, ["nas"]);
    expect(await cmdCrewApprovePromote(h.deps, ["--cancel"])).toBe(EXIT.OK);
  ...
  test("`crew status` shows a live approval on the lead, and never on a peer", async () => {
    ...
    await cmdCrewStatus(armed.deps, ["--no-probe"]);
    ...
    await cmdCrewStatus(stale.deps, ["--no-probe"]);
    ...
    await cmdCrewStatus(onPeer.deps, ["--no-probe"]);
...
describe("collie crew set-address", () => {
  test("rewrites the row, prints before → after, and restarts this lead", async () => {
    ...
    expect(await cmdCrewSetAddress(h.deps, ["nas", "nas.tail.ts.net:8788"])).toBe(EXIT.OK);
  ...
  test("a scheme is refused with the reason, and nothing is written", async () => {
    const h = harness(leadStore({ peers: [member({ memberId: "nas" })] }));
    expect(await cmdCrewSetAddress(h.deps, ["nas", "https://collie.example.com"])).toBe(EXIT.USAGE);
  ...
  test("a portless address is refused — it would dial :443, not the peer's listener", async () => {
    const h = harness(leadStore({ peers: [member({ memberId: "nas" })] }));
    expect(await cmdCrewSetAddress(h.deps, ["nas", "nas.tail.ts.net"])).toBe(EXIT.USAGE);
  ...
  test("the four refusals: not the lead, unknown member, this machine, and no arguments", async () => {
    ...
    expect(await cmdCrewSetAddress(peer.deps, ["desk", "desk.other:8787"])).toBe(EXIT.STATE);
    expect(text(peer.io)).toContain("collie reconnect <address>");

    const lead = harness(leadStore({ peers: [member({ memberId: "nas" })] }));
    expect(await cmdCrewSetAddress(lead.deps, ["ghost", "ghost.example:1"])).toBe(EXIT.STATE);
    expect(await cmdCrewSetAddress(lead.deps, ["desk", "desk.example:1"])).toBe(EXIT.STATE);
    expect(text(lead.io)).toContain("is this machine");
    expect(await cmdCrewSetAddress(lead.deps, ["nas"])).toBe(EXIT.USAGE);
    expect(lead.restarts).toEqual([]);
  });

  test("the address it already has is a no-op, not a write and not a restart", async () => {
    const h = harness(leadStore({ peers: [member({ memberId: "nas" })] }));
    expect(await cmdCrewSetAddress(h.deps, ["nas", "nas.example:8787"])).toBe(EXIT.OK);
...
describe("collie crew", () => {
  test("an unknown subcommand exits 2 and lists the real ones", async () => {
    const h = harness(leadStore());
    expect(await cmdCrew(h.deps, ["nonsense"])).toBe(EXIT.USAGE);
  ...
  // F20: `collie crew --help` answered `error: unknown crew subcommand \`--help\``. It is a spelling
  // of the block `collie crew` already prints, not a typo and not a second surface.
  test("`help`, `--help` and `-h` all print the block, and accuse nobody", async () => {
    const bare = harness(leadStore());
    expect(await cmdCrew(bare.deps, [])).toBe(EXIT.USAGE);
    for (const spelling of ["help", "--help", "-h"]) {
      const h = harness(leadStore());
      expect(await cmdCrew(h.deps, [spelling])).toBe(EXIT.USAGE);
      expect(text(h.io)).not.toContain("unknown crew subcommand");
  ...
  test("it routes to the verbs", async () => {
    const h = harness(leadStore({ peers: [member({ memberId: "nas" })] }));
    expect(await cmdCrew(h.deps, ["status", "--no-probe"])).toBe(EXIT.OK);
    expect(text(h.io)).toContain("mode   lead");
    expect(await cmdCrew(h.deps, ["approve-promote", "nas"])).toBe(EXIT.OK);
