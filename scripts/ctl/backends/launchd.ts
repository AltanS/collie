...
export type LaunchdRetryWaiter = (milliseconds: number) => Promise<void>;
...
const LAUNCHD_BOOTSTRAP_ATTEMPTS = 3;
const LAUNCHD_BOOTSTRAP_RETRY_MS = 1000;

async function bootstrapLaunchd(
  ctx: Ctx,
  options: LaunchdBootstrapOptions,
): Promise<void> {
  const args = ["bootstrap", options.domain, options.agentFile];
  for (let attempt = 1; attempt <= LAUNCHD_BOOTSTRAP_ATTEMPTS; attempt += 1) {
    const result = await ctx.shell("launchctl", args);
    if (result.exitCode === 0) return;
    if (attempt < LAUNCHD_BOOTSTRAP_ATTEMPTS) {
      await options.retryWait(LAUNCHD_BOOTSTRAP_RETRY_MS);
      continue;
    }
    throw shellFailure("launchctl", args, result);
  }
}
...
export function createLaunchdBackend(options: LaunchdBackendOptions = {}): InstalledServiceBackend {
  ...
  const bootstrapOptions: LaunchdBootstrapOptions = {
    domain,
    agentFile,
