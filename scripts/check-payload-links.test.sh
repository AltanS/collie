#!/usr/bin/env bash
# Tests for scripts/check-payload-links.sh — the guard that keeps a /nix/store link out of a
# released binary (#184).
#
...
TOOLS="$(with_tools tools bash cat env grep sed uname)"
fake_otool "$TOOLS"
fake_readelf "$TOOLS"
...
OUT
run_check "$TOOLS" CHECK_PAYLOAD_OS=Darwin FAKE_OTOOL_OUT="${TMP_ROOT}/otool-clean" \
  || fail "a system-linked Mach-O must pass: $OUT"
assert_contains "$OUT" "✓"
...
/tmp/collie:
	...
	/usr/lib/libSystem.B.dylib (compatibility version 1.0.0, current version 1351.0.0)
OUT
if run_check "$TOOLS" CHECK_PAYLOAD_OS=Darwin FAKE_OTOOL_OUT="${TMP_ROOT}/otool-nix"; then
...
# ── 3. Linux, an ordinary glibc payload ─────────────────────────────────────
...
run_check "$TOOLS" CHECK_PAYLOAD_OS=Linux \
  ...
  || fail "a system-linked ELF must pass: $OUT"
assert_contains "$OUT" "✓"
assert_contains "$OUT" "readelf"

# ── 4. Linux, an interpreter inside the store ───────────────────────────────
...
BARE="$(with_tools bare bash cat env grep sed uname)"
if run_check "$BARE" CHECK_PAYLOAD_OS=Darwin FAKE_OTOOL_OUT="${TMP_ROOT}/otool-clean"; then
