...
4. **No modal is on screen.** Every specific dialog grammar runs over the whole screen and none may
   ...
   paints: a `❯` anywhere, a rule, a stepper header, a numbered option, or a `<key> to <verb>` hint.
...
**The popup grammar also reads a clipped name.** A name may start with `…` as well as `/`. The
description column still has to agree, so the grammar is no looser. `@` file mentions stay out: no
capture of that popup exists.

## Consequences

- **Safety now comes from the frame and the modal checks.** Neither is a row count. A stale box
  above a live dialog is refused by the dialog, at any height.
...
- **An unrecognised modal with none of the step 4 marks would read as a live composer.** No such
  ...
  hint in its rows. A new modal shape is a new capture and a new grammar, as before.
- **The echo limitation in ADR 0004 is unchanged.** An echo of our own send, bracketed by rules,
  with a statusline-sized run below, still reads as a draft. `draftCarriesSend` still cannot tell.
