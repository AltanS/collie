...
// claude (full lift), omp (Tier 1 chrome strip), and grok (Tier 1 chrome plus Tier-2 probed
// permission / ask / plan lifts). `hasBlockGrammar`
// replaces the old grammar/agents predicate:
...
const ADAPTERS: Record<string, HarnessAdapter> = Object.fromEntries(
  [claudeAdapter, grokAdapter, ompAdapter].map((a) => [a.agent, a]),
);
