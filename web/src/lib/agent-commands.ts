...
// Sourced from the official docs (Claude Code: code.claude.com/docs; Codex: developers.openai.com
// /codex + openai/codex) and curated for one-tap use from a phone. A slash command is just text:
...
const CATALOG: Record<string, AgentCommand[]> = {
  claude: CLAUDE,
  codex: CODEX,
};
 ...
 */
export function commandsFor(agent: string | undefined | null): AgentCommand[] {
  if (!agent) return [];
  const key = agent.toLowerCase().trim();
  if (CATALOG[key]) return CATALOG[key];
  // Tolerate variants like "claude-code".
