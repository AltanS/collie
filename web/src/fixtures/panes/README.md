...
| `claude--menu-effort-slider--w60-ultracode.txt` | 60 × 40, `ultracode` selected, captured live 2026-09-22: **declines.** With `ultracode` selected the dialog repaints flush-left and unwrapped, so the terminal soft-wraps the footer at indent 0 on its second row against indent 3 on its first; `readKeyHintFooter`'s equal-indent rule refuses the group and the `←/→` phrase is lost. The generic `menu` grammar still lifts the screen, with a single Cancel action |
