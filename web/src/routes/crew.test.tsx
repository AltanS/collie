...
function renderCrew(data: CrewData, entry = "/crew", servers?: ServerSummary[]) {
...
const loaded: CrewData = { status: fixtureCrewStatus, error: false };

/** A roster entry with only the fields the layout reads — the geometry knows nothing else. */
function member(id: string, isLead = false): CrewMemberStatus {
...
describe("CrewRoute", () => {
  ...
  it("draws no deputy node when nobody is named", async () => {
    renderCrew({ status: { ...fixtureCrewStatus, deputy: null }, error: false });
  ...
  it("captions the drawing with the crew, its size and how much of it answers", async () => {
    renderCrew(loaded);
  ...
  it("puts the crew-wide facts on the LEAD's sheet, and its own word on each member's", async () => {
    const user = userEvent.setup();
    renderCrew(loaded);
  ...
  it("shouts about a conflict, a stale secret and a member never reached", async () => {
    ...
    expect(screen.getByText("crew protocol 2 (this collie speaks 1)")).toBeInTheDocument();
  });
...
describe("the entry points", () => {
  ...
  it("offers the census from the switcher sheet on a crew", async () => {
    const user = userEvent.setup();
    const router = createMemoryRouter(
      [
...
describe("CrewRoute — the per-host tint", () => {
  ...
  it("tints each node's glyph with the same colour that machine wears on the dashboard", async () => {
    ...
    renderCrew(loaded, "/crew", fixtureServers);
