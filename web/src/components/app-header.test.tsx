...
describe("AppHeader — the one shared header shell", () => {
  ...
  it("is calm in the PANE variant while live — breadcrumb + status badge, no pill, no wordmark", () => {
    // Quiet by default: a healthy pane header shows its own bits but NO connection pill.
    renderHeader(
      <AppHeader
        online
        bridge="connected"
    ...
    expect(screen.queryByRole("status")).toBeNull(); // no pill while live
  ...
  it("brings the SAME shared pill into BOTH variants only on an outage (pane + dashboard)", () => {
    // The whole point of the extraction: the same pill surfaces identically on every screen — but only
    // when not live. On a refresh error it appears amber "reconnecting…" in the pane header…
    const { unmount } = renderHeader(
      <AppHeader online bridge="connected" error onHome={() => {}} rightLead={<StatusBadge status="working" />}>
        <span>webapp › main</span>
      </AppHeader>,
    );
    expect(screen.getByRole("status")).toHaveTextContent("reconnecting…");
    unmount();
    // …and identically in the dashboard header (wordmark + gear).
    renderHeader(
      <AppHeader online bridge="connected" error wordmark rightTrail={<SettingsGear />} />,
    );
    expect(screen.getByText("Collie")).toBeInTheDocument();
    expect(screen.getByRole("status")).toHaveTextContent("reconnecting…");
  });

  it("is calm in the DASHBOARD variant while live — wordmark + settings gear, no pill", () => {
    renderHeader(
  ...
  it("the find-bar override takes over the whole row (mark, breadcrumb, and pill all yield)", () => {
    // `error` → not live, so the pill WOULD show — proving the override hides it, not just a live-quiet.
    renderHeader(
      <AppHeader
        online
        bridge="connected"
    ...
    // …so the normal content (including the outage pill) is replaced, not stacked.
