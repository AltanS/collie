...
import type { CrewMemberStatus, CrewStatusResponse, ServerSummary } from "@/lib/types";
import { cn } from "@/lib/utils";

// The crew census drawn as a FORMATION rather than listed as rows.
//
...
// ones — so "there is no deputy" is a missing row you cannot fail to notice, and a crew whose peers
...
// the state that needs nothing would make every well-behaved crew look like it wanted something. So
...
// and reads no clock, so the shape of a 1-, 2-, 3- and 7-machine crew is unit-testable without a
 ...
 * and SKIPPED entirely if none is, so the crew that named nobody is visibly one tier shallower
...
export function formationLayout(
  members: readonly CrewMemberStatus[],
  ...
  // The fan's own origin, so a crew with no lead at all (a payload we do not produce, but must not
  ...
  for (const [i, member] of peers.entries()) {
    ...
    // Left first: on a two-machine-plus-deputy crew the single peer sits left of centre, which is
 ...
 *
 * Exported because the fallback below is the only reader that has a `CrewMemberStatus` and needs a
...
export function asServerSummary(m: CrewMemberStatus): ServerSummary {
  return {
    ...
    // they lead the same crew is the one state where a write could land somewhere unintended.
 ...
 * never disagree. The fallback is for a mount with no `CrewProvider` — this route's own unit tests,
 ...
 * to go fix, `conflicted` is two collies claiming the same crew. Below them the presented state
 ...
 */
function ringStyle(m: CrewMemberStatus, health: HostHealth): RingStyle {
...
export function CrewFormation({ status, health, counts, servers, onSelect }: CrewFormationProps) {
  useLocale();
  ...
  return (
    <div className="flex flex-col items-center gap-3">
      <svg
        ...
        className="w-full"
        role="group"
        aria-label={t("crew.formation.aria", {
          machines: tn("crew.summary.machines", status.members.length),
        })}
      >
 ...
 * would read as "these machines report to the deputy" — a claim the crew protocol does not make.
