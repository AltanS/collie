...
describe("Motion section", () => {
  it("renders every card, no error boundary, unique flat-kebab handles", async () => {
  ...
  it("app-walkthrough: the real app navigates, and the readout follows it", async () => {
    ...
    await waitFor(() => expect(routeLine()).toContain("route: /"));

    fireEvent.click(within(card).getByRole("button", { name: "Pane" }));

    await waitFor(() => expect(routeLine()).toContain("route: /pane/"));
    // The pane's own header, portalled into the one shell header by the real DetailRoute.
    expect(await within(card).findByRole("button", { name: "Pane actions" })).toBeInTheDocument();
  ...
  it("app-walkthrough: space, tab and pane chips inside the app all lead somewhere", async () => {
    render(<MotionSection />);
    const card = cardFor("app-walkthrough");
    const routeLine = () =>
      card.querySelector('[data-slot="walkthrough-route"]')?.textContent ?? "";
    ...
    fireEvent.click(await within(card).findByRole("button", { name: /collie.+pane/i }));
    await waitFor(() => expect(routeLine()).toContain("route: /space/"));

    // A tab chip filters the space in place, so the route must NOT move off the space.
    const tabs = await within(card).findByRole("navigation", { name: "Tabs" });
    fireEvent.click(within(tabs).getByRole("button", { name: /docs/i }));
    await waitFor(() => expect(within(card).getByText(/ARCHITECTURE/)).toBeInTheDocument());
  ...
  it("collapse-primitive: toggling flips the Collapse root's data-state", async () => {
    render(<MotionSection />);
