...
// THE DASHBOARD'S FOOTER ON A SMALL PHONE (ADR 0066). Three tabs at 375x812: Panes, Attention,
// Changes. The claims a real engine has to check: a switch moves neither the footer nor the summary
// line, Attention shows only what needs you (and the all-clear when nothing does), and Changes lists
...
const ATTENTION = new RegExp(`^${en["home.tabs.attention"]}`, "u");
...
test("switching tabs moves neither the footer nor the summary line", async ({ page }) => {
  ...
  for (const name of [ATTENTION, CHANGES, PANES]) {
...
test("Attention shows only the panes that need you, and survives a reload", async ({ page }) => {
  await page.goto("/");
  ...
  await page.reload();
  await expect(tab(page, ATTENTION)).toHaveAttribute("aria-current", "page");
  await expect(page.getByRole("heading", { name: "collie" })).toHaveCount(0);
});

test("Attention with nothing urgent shows the all-clear line, not an empty list", async ({ page }) => {
