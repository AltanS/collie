...
/** The three machines `fixturePackStatus` describes: the lead, the deputy, and the conflicted one. */
const LEAD = fixturePackStatus.members[0]!;
const DEPUTY = fixturePackStatus.members[1]!;
...
test("the crew page names its lead and its deputy out loud", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { name: en["pack.title"] })).toBeVisible();
  ...
  await expect(
    page.getByRole("group", {
      name: fill(en["pack.formation.aria"], {
        machines: fill(en["pack.summary.machines.other"], {
          count: fixturePackStatus.members.length,
        }),
      }),
    }),
  ).toBeVisible();
  ...
  await expect(
    page.getByRole("button", {
      name: fill(en["pack.node.aria"], {
        name: LEAD.name,
        role: en["connection.host.lead"],
        health: en["pack.health.reachable"],
      }),
    }),
  ).toBeVisible();
  await expect(
...
test("Settings offers a way into the crew and into the updates", async ({ page }) => {
  ...
  await expect(page.getByRole("button", { name: en["pack.entry.title"] })).toBeVisible();
...
test("the dashboard footer says crew", async ({ page }) => {
  ...
  const line = page.getByRole("button", { name: en["pack.footer.aria"] });
  await expect(line).toBeVisible();
  await expect(line).toHaveText(
    fill(en["pack.footer.label"], {
      machines: fill(en["pack.summary.machines.other"], { count: fixtureServers.length }),
      reachable: fill(en["pack.summary.reachable"], {
...
test("in Japanese the crew page is called クルー", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { name: ja["pack.title"] })).toBeVisible();
  await expect(page.getByRole("button", { name: ja["pack.nav.back"] })).toBeVisible();
  expect(ja["pack.title"]).not.toBe(en["pack.title"]);
  expect(ja["pack.nav.back"]).not.toBe(en["pack.nav.back"]);
});
