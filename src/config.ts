...
export interface Config {
   ...
   * check meaningless (see CONCEPT §7). Override only if you know exactly why.
   */
  host: string;
  ...
  submitKeys: string[];
  /**
   * Tailscale identity gate. If set, requests must carry a matching `Tailscale-User-Login`
   * header (injected by `tailscale serve`). Empty = trust any loopback caller (fine when only
   * tailscaled can reach the port).
   */
  trustedUser: string;
  /** Extra allowed WebSocket origins beyond localhost (e.g. your MagicDNS https origin). */
...
export function loadConfig(): Config {
  ...
  return {
    ...
    submitKeys: envList("COLLIE_SUBMIT_KEYS").length
      ? envList("COLLIE_SUBMIT_KEYS")
      : ["Enter"],
