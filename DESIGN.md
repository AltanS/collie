# Design — Collie (a Herdr web bridge over Tailscale)
...
> still speak in the original "P0/P1" planning tense; treat those as historical. For the
> operational truth see [`README.md`](./README.md), [`CLAUDE.md`](./CLAUDE.md), and the verified
> socket API in [`HERDR_API.md`](./HERDR_API.md).
...
- serves a **mobile-first web app** (HTTP for assets, WebSocket for live state),
...
The counsel was unanimous here. A plugin **pane** runs inside a terminal pane: if the pane
closes, the user detaches, or Herdr restarts, the bridge dies — exactly when you're on mobile
...
Product details the counsel insisted on:
...
- **Validate the WebSocket `Origin`** on upgrade — reject anything that isn't the exact MagicDNS
  origin. Otherwise any page on any tailnet device can CSRF the socket. One line of code.
