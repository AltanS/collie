...
import type { CrewChange, RosterEntry } from "./enrollment.ts";
import { fingerprintOfCert } from "./identity.ts";
import type { CrewLink, PeerOutcome } from "./peer-client.ts";
import { dialTls, type CrewTlsOptions } from "./transport.ts";
...
// two-machine crew has no witness, is allowed anyway, and the page says so (RFC §16, decision 8) —
...
export type TakeoverRefusal =
  /** This collie is not a peer of the warrant's issuer, or is in no crew at all. */
  | "not-a-peer"
 ...
 *
 * Every clause is a question about material this collie already holds — its own crew id, the
 ...
 * §10.1's rule is that there is exactly one silence clock in the crew and every reader of it reads
 * the same number.
 */
 ...
 *
 * The member id, the crew identity, the crew secret and this collie's own key material are all
 ...
 * roster entry of its own — its lead — so without it a takeover would be a takeover into a crew it
 ...
 *
 * **The warrant is kept, and the designation is dropped.** The crew has no deputy after a takeover
...
export function clearRePin(data: TrustStoreData, memberId: string): CrewChange<{ readonly member: string }> | null {
 ...
 *   • The **LEAD** is dialled with no TLS material at all. `bridge/crew/transport.ts`'s design note
 *     states the law: "A LEAD does not pin its listener at all. Its crew surface rides the front
 ...
 * and a two-machine crew has no witness to catch it (RFC §16, decision 8): the crown would be taken
 ...
 * nobody's roster, so a §8.6 body signature could only fail to verify), but the crew secret and the
 ...
 */
export function takeoverDialTls(data: TrustStoreData | null, memberId: string): CrewTlsOptions | undefined {
...
export function takeoverMessage(outcome: TakeoverOutcome): string {
  ...
  if (outcome.reason === "no-roster") {
    return "This machine holds no roster for the crew, so it cannot lead one. Re-run `collie crew deputy` on the lead. Nothing was changed.";
  }
