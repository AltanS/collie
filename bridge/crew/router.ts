 ...
 */
export const CREW_MUX_FIELD = "mux";
...
// receiving half of a verb in `cli/crew.ts`; none is reachable by a browser, and all three sit behind
 ...
 * prose because the LEAD is the machine whose operator can fix it, and `crew status` there has to
 ...
 * transport cannot pin (the lead's front door terminates TLS, `bridge/crew/transport.ts`). The two
 * membership routes are why the mechanism exists; `hello` is on the list because `collie crew status`
 ...
 *
 *   • {@link CREW_TAKEOVER_PATH} — the witness question and the re-pin (RFC §7);
 *   • {@link CREW_WARRANT_PATH} — RFC §9's reconciliation, and it is the SAME decision as a commit.
 ...
 * the next.
 */
const DEPUTY_ROUTES: ReadonlySet<string> = new Set<string>([CREW_TAKEOVER_PATH, CREW_WARRANT_PATH]);
...
export interface StandbySurface {
  ...
  /** Gap A's silence, in ms — **the same number** `crew status` and the door read (RFC §10.1). */
   ...
   * `crew status` on the LEAD — the machine whose operator can rename the device — sees it the moment
...
export interface CrewRouterDeps {
   ...
   * (`bridge/crew/transport.ts`). **Defaults to `false`, which admits nothing but a signed request.**
   *
   ...
   * with one (`bridge/crew/transport.ts` → `deputyAnchorOf`). Absent on every single-anchor peer, on
   ...
   * `ca` at all — so what this hook buys is the process SAYING so (bridge/crew/staleness.ts). It is a
   ...
   * far side and deliberately never persisted (`bridge/crew/lead-contact.ts` says why).
   */
   ...
   * forbidden to dial from it — the next sweep tick does that (`CrewLead.noteAdmittedContact`).
   */
  readonly onMemberDialled?: (memberId: string) => void;
  /**
   * The pinned lead was **identified and refused on the crew secret** — §8.4's rotation, seen from
   ...
   *
   * A notification, exactly like {@link CrewRouterDeps.onMembershipChange} — the bridge does not
   ...
   * only from a verdict probe — which a healthy crew never fires.
   *
   * **Threaded in once, at boot, by whoever constructs the router** (`bridge/index.ts`). It is not
   * read per request: the answer cannot change without a restart, and `hello` is the crew's most
   ...
   * anything (`bridge/crew/follow.ts` holds all eight guards), and a build that supplies no
...
/** Answers a crew request, or `null` when the path is not ours (so the normal router continues). */
export type CrewHandler = (req: Request, url: URL) => Promise<Response | null>;
 ...
 * `deriveMode` correctly calls it `solo` (bridge/crew/mode.ts) — yet it must be able to *answer* that
 * invite or a crew can never form. Tying registration to `mode !== "solo"` would make the first
...
type HelloBody = {
  ...
  /** This peer's own capability declaration (§7.1's amendment of 2026-09-08). See {@link CREW_MUX_FIELD}. */
  mux?: MuxConfig;
};
...
/**
 * A box rather than a bare `let`, so a refusal decided inside `commitCrewChange`'s callback survives
...
function badRequest(self: string, reason: string): Response {
  return new Response(JSON.stringify({ error: reason }), {
    status: 400,
    headers: crewResponseHeaders(self),
  });
}
...
function leadConflict(
  ...
  return new Response(JSON.stringify(body), { status: 409, headers: crewResponseHeaders(self) });
}
...
function takeoverRefusalText(reason: Exclude<TakeoverRefusal, "bad-signature">): string {
  ...
  if (reason === "foreign") return "this warrant is not from this collie's own lead, or not for this crew";
...
function warrantRefusalText(reason: Exclude<WarrantRefusal, "bad-signature">): string {
  ...
  if (reason === "foreign") return "this warrant is not from this collie's own lead, or not for this crew";
  if (reason === "expired") return "this warrant is past its validity on this collie's clock — re-run `collie crew deputy`";
...
export function createCrewRouter(deps: CrewRouterDeps): CrewHandler {
  ...
  async function answerCrew(
    ...
    if (signature !== null && data !== null) {
      ...
      // still comes from the dial attestation (`resolveDial` → `admitCrewRequest`), which is the same
    ...
    if (pathname === CREW_HELLO_PATH && req.method === "GET") {
      ...
      // for this member without forwarding a config read (ADR 0011 keeps the crew link the only
    ...
    if (pathname === CREW_SECRET_PATH && req.method === "POST") {
      return secret(req, signedBody, verdict.member, verdict.self);
    }
    if (pathname === CREW_WARRANT_PATH && req.method === "POST") {
      return warrant(req, signedBody, verdict.member, verdict.self);
    }
    if (pathname === CREW_PAIRING_PATH && req.method === "POST") {
    ...
    if (pathname === CREW_TAKEOVER_PATH && req.method === "POST") {
      ...
      return refuse(CREW_TAKEOVER_PATH, "not-a-crew-member");
    }
    if (pathname === CREW_LEAD_PATH && req.method === "POST") {
      return newLead(req, signedBody, verdict.member, verdict.self);
    }
    if (pathname === CREW_LEAVE_PATH && req.method === "POST") {
    ...
    if (pathname === CREW_SNAPSHOT_PATH && req.method === "GET" && deps.snapshot !== undefined) {
      // The only merged route (§9.2), and the peer's half of it: it answers with its OWN view,
      // never a merged one — a crew link never forwards a `host=`, because a peer has no peers (§4).
      ...
      // very object this collie serves its own browser, and a crew-only field has no business in the
      ...
      // does: `body` is the very object this collie serves its own browser, and a crew-only fact has
      ...
      const withPreflight =
        preflight === null ? withReport : { ...withReport, [CREW_PREFLIGHT_FIELD]: preflight };
      ...
      const withRun = ownRun === null ? withPreflight : { ...withPreflight, [CREW_RUN_FIELD]: ownRun };
  ...
  async function deputyAnswer(
    ...
    const applied = await commitCrewChange(deps.store, deps.audit, (current) =>
    ...
    return new Response(
      ...
      { status: 200, headers: crewResponseHeaders(self) },
    );
  }
  ...
  function handoverNotApproved(self: string, claimant: string): Response {
    return new Response(
      JSON.stringify({
        error:
          `this lead has not approved "${claimant}" to take over — run \`collie crew approve-promote ${claimant}\` ` +
          "here, then re-run `collie promote` on that machine within 10 minutes",
        code: HANDOVER_NOT_APPROVED,
      }),
      { status: 403, headers: crewResponseHeaders(self) },
    );
  }
   ...
   * out of its own crew — a compromised peer escalating to crew-wide denial (§8.5 is explicit that a
  ...
  async function secret(req: Request, cached: string | null, from: TrustedMember, self: string): Promise<Response> {
    ...
    if (data === null || data.lead === null || data.lead.memberId !== from.memberId) {
      ...
      return refuse(CREW_SECRET_PATH, "not-a-crew-member");
    }
    const applied = await commitCrewChange(deps.store, deps.audit, (current) =>
   ...
   * hands one transition to `commitCrewChange`; a warrant that does not verify leaves this collie
  ...
  async function warrant(req: Request, cached: string | null, from: TrustedMember, self: string): Promise<Response> {
    ...
    if (data === null || data.lead === null || data.lead.memberId !== from.memberId) {
      ...
      return refuse(CREW_WARRANT_PATH, "not-a-crew-member");
    }
    ...
    if (verdict.kind === "refuse") {
      if (verdict.reason === "bad-signature") {
        ...
        return refuse(CREW_WARRANT_PATH, "certificate");
      }
    ...
    const applied = await commitCrewChange(deps.store, deps.audit, (current) =>
      ...
      // dial (§4), and a deputy without one could not lead the crew it is about to inherit.
  ...
  async function pairingSync(
    ...
    // the adoption where it belongs, and the REPORT riding the sync so `crew status` on the lead names
    // the labels early, while the operator still has a healthy crew to rename them in.
   ...
   *      is signable;
   *   2. the warrant must be for this crew, name a deputy, and carry a generation at least this
  ...
  async function depose(
    ...
    if (!isDepositionProof(data, proof)) return refuse(CREW_WARRANT_PATH, "certificate");

    const heal = selfHeal(data, proof);
    const state = deposedStateFrom(data, proof, heal, now());
    if (heal.outcome === "healed") {
      await commitCrewChange(deps.store, deps.audit, (current) => (current === null ? null : heal.change));
    } else {
    ...
    // Announced, never silent (RFC §12, F11): a machine rejoining a crew by itself must be a thing
   ...
   *   • **a peer** re-pins and starts dialling the new address, keeping its member id and the crew
  ...
  async function newLead(req: Request, cached: string | null, from: TrustedMember, self: string): Promise<Response> {
    ...
    const data = await deps.store.load();
    if (data === null) return refuse(CREW_LEAD_PATH, "not-a-crew-member");
    ...
    if (isLeading(data)) {
      ...
      const handover = await commitCrewChange(deps.store, deps.audit, (current) => {
      ...
      if (handover === null) return badRequest(self, "not the lead of this crew");
    ...
    const changed = await commitCrewChange(deps.store, deps.audit, (current) =>
  ...
  async function enroll(req: Request, wire: number): Promise<Response> {
    ...
    const invite = await commitCrewChange(deps.store, deps.audit, (data) =>
      data === null ? null : consumeInvite(data, parsed?.token ?? null, now()),
    );
    if (invite === null || parsed === null) return refuse(CREW_ENROLL_PATH, "token");
    ...
    // under any design (`bridge/crew/transport.ts`). What vouches for the certificate is the
    ...
    const response = await commitCrewChange(deps.store, deps.audit, (data) =>
      data === null
        ? null
        : enrollPeer(
            data,
    ...
    if (response === null) return refuse(CREW_ENROLL_PATH, "not-a-crew-member");
