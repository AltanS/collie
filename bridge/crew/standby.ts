...
// ── Configuration (crew keys, so they live here and never on `Config`) ───────
 ...
 * idle crew arms its own standby door every night. The threshold MUST exceed `COLLIE_POLL_IDLE_MS` or
 * an idle crew arms itself, and the formula guarantees it.
 *
...
export interface StandbyFacts {
  /**
   * This machine holds a **verified** stored warrant naming ITSELF: for this crew, issued by the
  ...
  /** Gap A's number, and there is only one of it (RFC §10.1) — `crew status` prints this same value. */
 ...
 *
 *   • for THIS crew, and issued by the member this collie pins as its lead;
 *   • naming a deputy at all — a revocation names nobody and arms nobody (§18.3);
 *   • naming THIS collie;
 *   • unexpired on THIS machine's own clock (§18.4) — a dark crew disarms itself here as well as at
 *     the transport;
...
/** Gap A's silence, from the one holder, so the door and `crew status` can never read two clocks. */
 ...
 *
 * A member id is slugified at `join` and a crew name is operator-typed, so neither is plausibly
 ...
 *
 * A **two-machine crew says the quiet part above the button** (RFC §16, decision 8): there is nobody
 ...
 * is merely unreachable splits the crew.
 */
export function standbyPage(facts: StandbyFacts): string {
  ...
  const crew = facts.crewName === null ? "this crew" : `&ldquo;${escapeHtml(facts.crewName)}&rdquo;`;
  const body: string[] = [];
  ...
  if (report.armed) {
    body.push(
      ...
      `<p>Your lead ${lead} has not called this machine for ${humanSilence(facts.silentForMs)}. ` +
        `This machine (<code>${self}</code>) is the deputy of crew ${crew}.</p>`,
    );
    ...
    body.push(
      `<h1>Standby</h1>`,
      `<p>This machine (<code>${self}</code>) is the deputy of crew ${crew}. Nothing here is live.</p>`,
...
export function createStandbyDoor(deps: StandbyDoorDeps) {
  return async (req: Request, url: URL): Promise<Response | null> => {
    ...
    if (url.pathname === STANDBY_HEALTH_PATH) {
      ...
      // run when armed, and what this machine is running. No member ids, no crew name, no roster.
      //
...
export function frontDoorHealth(mode: CrewMode, url: URL): Response | null {
