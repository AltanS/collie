...
function call(
  handler: ReturnType<typeof createCrewRouter>,
  path: string,
  init: RequestInit = {},
...
describe("the prefix", () => {
  ...
  test("a non-crew path returns null so the ordinary router continues", async () => {
...
describe("GET /pack/v1/hello — behind both factors", () => {
  ...
  test("a machine with no adapter in hand omits the block, which reads as the lead's (M22/03)", async () => {
    ...
    const h = harness(leadStore({ peers: [nas] }));
    const handler = createCrewRouter({ store: h.store, audit: h.audit, now: () => T0, mux: () => null });
    const res = (await call(handler, CREW_HELLO_PATH, {
      headers: { ...authed, ...signed("nas", "GET", CREW_HELLO_PATH, "", T0) },
    }))!;
  ...
  test("without a pinned certificate it is 401 — the unwired default admits nobody", async () => {
    const h = harness(leadStore({ peers: [nas] }));
    const handler = createCrewRouter({ store: h.store, audit: h.audit });
    const res = (await call(handler, CREW_HELLO_PATH, { headers: authed }))!;
  ...
  test("every refusal cause produces the identical response (§8.1)", async () => {
    ...
    for (const [, headers] of cases) {
      const res = (await call(unpinned, CREW_HELLO_PATH, { headers }))!;
...
describe("GET /pack/v1/snapshot — the one merged route, §9.2", () => {
  ...
  test("an admitted caller gets the peer's own snapshot body verbatim, with the crew headers", async () => {
    const h = harness(peerStore());
    const body = ownSnapshot();
    const source: SnapshotSource = () => body;
    const handler = createCrewRouter({ store: h.store, audit: h.audit, transportPinned: true, snapshot: source });
  ...
  test("?session= is passed through to the injected source", async () => {
    ...
    const handler = createCrewRouter({ store: h.store, audit: h.audit, transportPinned: true, snapshot: source });
    await call(handler, `${CREW_SNAPSHOT_PATH}?session=collie-demo`, { headers: authed });
  ...
  test("?sessions=all reaches the injected source, and absent means today's narrow answer", async () => {
    ...
    const handler = createCrewRouter({ store: h.store, audit: h.audit, transportPinned: true, snapshot: source });
    await call(handler, `${CREW_SNAPSHOT_PATH}?sessions=all`, { headers: authed });
    await call(handler, `${CREW_SNAPSHOT_PATH}?session=work&sessions=all`, { headers: authed });
  ...
  test("an unknown session (source returns undefined) is the peer's OWN 404, not the lead's", async () => {
    ...
    const handler = createCrewRouter({ store: h.store, audit: h.audit, transportPinned: true, snapshot: source });
    const res = (await call(handler, `${CREW_SNAPSHOT_PATH}?session=nope`, { headers: authed }))!;
  ...
  test("an UNADMITTED caller gets the standard 401 and the snapshot source is NEVER invoked", async () => {
    ...
    const handler = createCrewRouter({ store: h.store, audit: h.audit, snapshot: source });
    const res = (await call(handler, CREW_SNAPSHOT_PATH, { headers: authed }))!;
    expect(res.status).toBe(401);
    expect(calls).toBe(0);
  });
  ...
  test("a member with more checks than the cap is truncated, and the truncation is stated", async () => {
    ...
    expect(carried).toHaveLength(CREW_PREFLIGHT_MAX_CHECKS);
    expect(carried.at(-1)!.id).toBe(CREW_PREFLIGHT_TRUNCATED_ID);
    expect(carried.at(-1)!.reason).toContain("not carried over the crew link");
  });
  ...
  test("updateRun rides beside the body, and is OMITTED when there is nothing to report", async () => {
    ...
    expect(await (await call(handler, CREW_SNAPSHOT_PATH, { headers: authed }))!.json()).toEqual({
      ...body,
      updateRun: report,
    });
    // The browser's own snapshot is untouched: a crew-only fact never leaks into it.
    ...
    expect(await (await call(silent, CREW_SNAPSHOT_PATH, { headers: authed }))!.json()).toEqual(body);
  });
  ...
  test("a non-GET method on the path falls through to the ordinary 404, not 405", async () => {
    ...
    const handler = createCrewRouter({ store: h.store, audit: h.audit, transportPinned: true, snapshot: source });
    const res = (await call(handler, CREW_SNAPSHOT_PATH, { method: "POST", headers: authed }))!;
    expect(res.status).toBe(404);
...
describe("POST /pack/v1/enroll — admitted by the TOKEN, not by the two factors", () => {
  ...
  test("F4: a junk enroll rewrites NOTHING — no store write, no audit line, unbounded and free to us", async () => {
    ...
    // garbage POST into a re-serialize of the file holding the private key and the crew secret.
    const h = invited();
    const handler = createCrewRouter({ store: h.store, audit: h.audit, now: () => T0 + 1 });
...
describe("dispatched routes — the peer runs its own routes for an admitted lead (§5)", () => {
  ...
  test("every §5 route reaches the dispatch as its own /api path, verbatim", async () => {
    ...
    for (const [route, method] of routes) {
      ...
      const res = (await call(handler, `${CREW_PREFIX}${route}`, init))!;
      expect(res.status).toBe(200);
    }
  ...
  test("`?session=` rides through untouched — the PEER's registry resolves it (§5)", async () => {
    ...
    const { handler } = peerRouter(d);
    await call(handler, `${CREW_PREFIX}pane/w1:p1?session=work&lines=80`, { headers: authed });
    expect(d.seen[0]!.search).toBe("?session=work&lines=80");
  });

  test("a crew request may NOT name a host — a peer has no peers (§4)", async () => {
    const d = dispatcher(() => new Response("{}"));
    const { handler } = peerRouter(d);
    const res = (await call(handler, `${CREW_PREFIX}pane/w1:p1?host=desk`, { headers: authed }))!;
  ...
  test("an unadmitted caller never reaches the dispatch — routing happens after both factors", async () => {
    const d = dispatcher(() => new Response("{}"));
    const h = harness(leadStore({ peers: [nas] }));
    const stranger = createCrewRouter({ store: h.store, audit: h.audit, dispatch: d.dispatch });
    const res = (await call(stranger, `${CREW_PREFIX}pane/w1:p1/reply`, { method: "POST", headers: authed }))!;
    expect(res.status).toBe(401);
    expect(d.seen).toEqual([]);
  });

  test("the answer keeps its own status and body, and gains the crew headers §6 requires", async () => {
    const d = dispatcher(() => new Response(`{"ok":false,"error":"no such pane"}`, { status: 404 }));
    const { handler } = peerRouter(d);
    const res = (await call(handler, `${CREW_PREFIX}pane/nope`, { headers: authed }))!;
    expect(res.status).toBe(404);
  ...
  test("a 304 survives the peer surface with its ETag and no body (§9.1)", async () => {
    ...
    const { handler } = peerRouter(d);
    const res = (await call(handler, `${CREW_PREFIX}pane/w1:p1`, {
  ...
  test("a build with no dispatch wired 404s the whole half of the table", async () => {
    ...
    expect((await call(handler, `${CREW_PREFIX}pane/w1:p1`, { headers: authed }))!.status).toBe(404);
  });
});

// ── The membership routes (M4/07) ────────────────────────────────────────────
// The receiving halves of `collie crew rotate`, `collie promote` and `collie leave`. Each one is
...
describe("POST /pack/v1/secret — the peer side of rotation (§8.4)", () => {
  ...
  const asLead = (h: ReturnType<typeof harness>) =>
    createCrewRouter({ store: h.store, audit: h.audit, transportPinned: true, now: () => T0 });
...
describe("POST /pack/v1/lead — the promotion handover (§14)", () => {
  ...
  test("the old lead demotes itself and answers with its roster", async () => {
    ...
    const handler = createCrewRouter({ store: h.store, audit: h.audit, now: () => T0 });
    const res = (await call(handler, CREW_LEAD_PATH, signedPost("nas", CREW_LEAD_PATH, { lead: claim }, T0)))!;
    expect(res.status).toBe(200);
    expect(await res.json()).toEqual({
      demoted: "desk",
  ...
  test("an UNAPPROVED claim is refused 403, and the store is not written at all", async () => {
    ...
    expect(await res.json()).toEqual({
      error:
        'this lead has not approved "nas" to take over — run `collie crew approve-promote nas` here, then ' +
  ...
  test("consent names the certificate: an approved member claiming under another key is refused", async () => {
    ...
    const res = (await call(handler, CREW_LEAD_PATH, signedPost("nas", CREW_LEAD_PATH, { lead: impostor }, T0)))!;
    expect(res.status).toBe(403);
    ...
    expect(((await res.json()) as CrewRefusal).code).toBe("handover_not_approved");
    expect(h.data().lead).toBeNull();
  ...
  test("a peer re-pins the new lead and answers with an empty roster — it has no peers", async () => {
    ...
    const h = harness(peerStore());
    const handler = createCrewRouter({ store: h.store, audit: h.audit, transportPinned: true, now: () => T0 });
    const relayed = { memberId: "desk", fingerprint: fp("desk"), certPem: material("desk").certPem, address: "desk.moved:8787" };
    const res = (await call(handler, CREW_LEAD_PATH, post({ lead: relayed })))!;
  ...
  test("a member may only claim leadership FOR ITSELF — nobody nominates a third party", async () => {
    const h = harness(peerStore());
    const handler = createCrewRouter({ store: h.store, audit: h.audit, transportPinned: true });
    const res = (await call(handler, CREW_LEAD_PATH, post({ lead: { ...claim, memberId: "nas" } })))!;
    expect(res.status).toBe(400);
  ...
  test("an unadmitted caller cannot move the crown", async () => {
    const h = harness(leadStore({ peers: [member({ memberId: "nas" })] }));
    const handler = createCrewRouter({ store: h.store, audit: h.audit });
    const res = (await call(handler, CREW_LEAD_PATH, post({ lead: claim })))!;
    expect(res.status).toBe(401);
    expect(h.data().lead).toBeNull();
  });
  ...
  test("a malformed claim is a 400 on an admitted link — it may say why", async () => {
    const h = harness(peerStore());
    const handler = createCrewRouter({ store: h.store, audit: h.audit, transportPinned: true });
    const res = (await call(handler, CREW_LEAD_PATH, post({ lead: { memberId: "desk" } })))!;
    expect(res.status).toBe(400);
...
describe("POST /pack/v1/leave — the caller drops ITSELF (§8.4)", () => {
  ...
  test("an admitted member removes its own roster entry and nothing else", async () => {
    const h = harness(leadStore({ peers: [member({ memberId: "nas" }), member({ memberId: "laptop" })] }));
    const handler = createCrewRouter({ store: h.store, audit: h.audit, now: () => T0 });
    const res = (await call(handler, CREW_LEAVE_PATH, signedPost("nas", CREW_LEAVE_PATH, { member: "laptop" }, T0)))!;
    expect(res.status).toBe(200);
  ...
  test("leaving twice is 200 both times — the operator's question has the same answer", async () => {
    const h = harness(leadStore({ peers: [member({ memberId: "nas" })] }));
    const handler = createCrewRouter({ store: h.store, audit: h.audit, now: () => T0 });
    expect((await call(handler, CREW_LEAVE_PATH, signedPost("nas", CREW_LEAVE_PATH, {}, T0)))!.status).toBe(200);
    ...
    expect((await call(handler, CREW_LEAVE_PATH, signedPost("nas", CREW_LEAVE_PATH, {}, T0)))!.status).toBe(401);
  });

  test("an unadmitted caller removes nobody", async () => {
    const h = harness(leadStore({ peers: [member({ memberId: "nas" })] }));
    const handler = createCrewRouter({ store: h.store, audit: h.audit });
    expect((await call(handler, CREW_LEAVE_PATH, post({})))!.status).toBe(401);
...
// roster at boot and does not re-read it. Nothing re-wires in place (bridge/crew/staleness.ts says
...
// private key and the crew secret, however many times it asks.
//
...
/** A secret no crew could have minted. `CREW.secret` is the only correct value in this file. */
...
describe("N1 — the crew secret alone: a wrong or missing one refuses, and changes nothing", () => {
  const claim = { memberId: "nas", fingerprint: fp("nas"), certPem: material("nas").certPem, address: "nas.example:8787" };
  ...
  test("a correctly-signed `leave` with the WRONG secret removes nobody, and never touches the disk", async () => {
    ...
    await h.store.load();
    const before = h.contents();
    const res = (await call(handler, CREW_LEAVE_PATH, signedPostWith("nas", CREW_LEAVE_PATH, {}, T0, WRONG_SECRET)))!;
    expect(res.status).toBe(401);
    expect(await res.json()).toEqual({ error: "unauthorized" });
  ...
  test("NO secret at all is the same refusal, with the same nothing behind it", async () => {
    ...
    const handler = createCrewRouter({ store: h.store, audit: h.audit, now: () => T0 });
    await h.store.load();
    const res = (await call(handler, CREW_LEAVE_PATH, signedPostWith("nas", CREW_LEAVE_PATH, {}, T0, null)))!;
    expect(res.status).toBe(401);
    ...
    const ok = (await call(handler, CREW_LEAVE_PATH, signedPost("nas", CREW_LEAVE_PATH, {}, T0)))!;
  ...
  test("this collie's own lead, on the PINNED transport, cannot rotate the secret without it (§8.4)", async () => {
    ...
    // refused. A rotation is the one route where a single-factor slip would hand the crew away.
    const h = harness(peerStore());
    const handler = createCrewRouter({ store: h.store, audit: h.audit, transportPinned: true, now: () => T0 });
    await h.store.load();
    const res = (await call(handler, CREW_SECRET_PATH, {
      method: "POST",
  ...
  test("an APPROVED handover still needs the secret — the operator's consent is not a factor (§14)", async () => {
    ...
    const res = (await call(
      handler,
      CREW_LEAD_PATH,
      signedPostWith("nas", CREW_LEAD_PATH, { lead: claim }, T0, WRONG_SECRET),
    ))!;
  ...
  test("a wrong secret never reaches the dispatched handlers — nothing types into a terminal", async () => {
    ...
    for (const route of ["pane/w1:p1", "pane/w1:p1/reply", "pane/w1:p1/keys"]) {
      ...
      expect((await call(handler, `${CREW_PREFIX}${route}`, init))!.status).toBe(401);
    }
    expect(seen).toEqual([]);
    expect(h.writes()).toBe(0);
  });
...
describe("N2 — the pinned certificate alone: an identity this collie does not pin refuses", () => {
  // The TLS handshake itself needs a live `Bun.serve` and cannot run here (bridge/crew/transport.ts
  ...
  test("a signature from a certificate this collie does not pin admits nothing, and writes nothing", async () => {
    ...
    const res = (await call(
      handler,
      CREW_LEAVE_PATH,
      signedPostWith("stranger", CREW_LEAVE_PATH, {}, T0, CREW.secret),
    ))!;
    expect(res.status).toBe(401);
    expect(h.data().peers.map((p) => p.memberId)).toEqual(["nas", "laptop"]);
    expect(h.writes()).toBe(0);
  ...
  test("`X-Pack-Member` is a hint, never an identity — naming a pinned member does not admit a stranger", async () => {
    ...
    const res = (await call(
      handler,
      CREW_LEAVE_PATH,
      signedPostWith("stranger", CREW_LEAVE_PATH, {}, T0, CREW.secret, "nas"),
    ))!;
    expect(res.status).toBe(401);
    expect(h.data().peers.map((p) => p.memberId)).toEqual(["nas"]);
    expect(h.writes()).toBe(0);
  ...
  test("the admitted member is the KEY that signed, never the member the header claims", async () => {
    ...
    const res = (await call(
      handler,
      CREW_LEAVE_PATH,
      signedPostWith("laptop", CREW_LEAVE_PATH, {}, T0, CREW.secret, "nas"),
    ))!;
    expect(res.status).toBe(200);
  ...
  test("an `unenrolled` pin is not an identity — a member dropped by a rotation cannot sign its way back", async () => {
    ...
    const handler = createCrewRouter({ store: h.store, audit: h.audit, now: () => T0 });
    await h.store.load();
    const res = (await call(handler, CREW_LEAVE_PATH, signedPost("nas", CREW_LEAVE_PATH, {}, T0)))!;
    expect(res.status).toBe(401);
  ...
  test("an unpinned transport admits nobody, however correct the secret — and a pinned one admits", async () => {
    ...
    // "the pin could not be built". Fail-closed: the correct crew secret buys nothing on its own.
    const h = harness(peerStore());
    await h.store.load();
    const unpinned = createCrewRouter({ store: h.store, audit: h.audit, now: () => T0 });
    const rotation = {
      method: "POST",
      headers: withSecret(CREW.secret),
...
describe("POST /pack/v1/warrant — the receiving half of the deputy designation (§18)", () => {
  ...
  const asLead = (h: ReturnType<typeof harness>, now = T0) =>
    createCrewRouter({ store: h.store, audit: h.audit, transportPinned: true, now: () => now });
  ...
  test("this collie's own lead delivers one, and it lands on disk with the deputy's certificate", async () => {
    ...
    const res = (await call(asLead(h), CREW_WARRANT_PATH, post(pushBody(w))))!;
    expect(res.status).toBe(200);
  ...
  test("a redelivery answers 200, applies nothing, and does not touch the disk twice", async () => {
    const h = harness(peerStore());
    const w = warrantFor("nas");
    await call(asLead(h), CREW_WARRANT_PATH, post(pushBody(w)));
    const before = h.writes();
    const res = (await call(asLead(h), CREW_WARRANT_PATH, post(pushBody(w))))!;
    expect(await res.json()).toEqual({ generation: 1, applied: false });
  ...
  test("a SIGNED push advances this member's replay floor — the same stamp twice is refused", async () => {
    // The mechanism behind the `crew deputy` live-drill failure of 2026-08-20, pinned where it
    // lives. `CREW_WARRANT_PATH` is in `MEMBERSHIP_PATHS`, so an admitted SIGNED push commits
    ...
    // `collie crew deputy` verb), so their stamps can collide and the second body to land is refused
    // as a replay. It is a stamp collision, not a permission failure, and the remedy is a retry with
    // a fresh stamp — which is what `cli/crew-deputy.ts` now does.
    const h = harness(peerStore());
    const w = warrantFor("nas");
    const first = (await call(asLead(h), CREW_WARRANT_PATH, signedPost("desk", CREW_WARRANT_PATH, pushBody(w), T0 + 2)))!;
    ...
    for (const stamp of [T0 + 2, T0 + 1]) {
      const again = (await call(asLead(h), CREW_WARRANT_PATH, signedPost("desk", CREW_WARRANT_PATH, pushBody(w), stamp)))!;
    ...
    const retry = (await call(asLead(h), CREW_WARRANT_PATH, signedPost("desk", CREW_WARRANT_PATH, pushBody(w), T0 + 3)))!;
  ...
  test("an OLD generation is refused, and the newer one it holds is what it reports", async () => {
    ...
    await call(asLead(h), CREW_WARRANT_PATH, post(pushBody(second)));
    const res = (await call(asLead(h), CREW_WARRANT_PATH, post(pushBody(first))))!;
  ...
  test("a certificate that is not the one the fingerprint names is a 400, and nothing is stored", async () => {
    const h = harness(peerStore());
    const res = (await call(asLead(h), CREW_WARRANT_PATH, post(pushBody(warrantFor("nas"), "attic"))))!;
    expect(res.status).toBe(400);
  ...
  test("a tampered warrant is the uniform 401 — a bad signature is told nothing more", async () => {
    ...
    const res = (await call(asLead(h), CREW_WARRANT_PATH, post(pushBody(forged))))!;
    expect(res.status).toBe(401);
    expect(h.data().warrant).toBeUndefined();
    expect(h.writes()).toBe(0);
    expect(h.lines.map((l) => l.action)).toContain("pack.refused");
  ...
  test("one past its 30 days is refused on THIS collie's clock, however well it verifies", async () => {
    const h = harness(peerStore());
    const res = (await call(asLead(h, T0 + WARRANT_TTL_MS), CREW_WARRANT_PATH, post(pushBody(warrantFor("nas")))))!;
    expect(res.status).toBe(400);
    expect(h.data().warrant).toBeUndefined();
  });
  ...
  test("a revocation names nobody, carries nothing, and still lands", async () => {
    ...
    const res = (await call(asLead(h), CREW_WARRANT_PATH, post({ warrant: revoked })))!;
    expect(await res.json()).toEqual({ generation: 2, applied: true });
  ...
  test("a malformed body is a 400 and writes nothing", async () => {
    ...
    for (const body of [{}, { warrant: "nope" }, { warrant: { generation: 1 } }]) {
      const res = (await call(asLead(h), CREW_WARRANT_PATH, post(body)))!;
      expect(res.status).toBe(400);
    }
    expect(h.writes()).toBe(0);
  });
  ...
  test("an unadmitted caller cannot reach it at all", async () => {
    const h = harness(peerStore());
    const handler = createCrewRouter({ store: h.store, audit: h.audit });
    const res = (await call(handler, CREW_WARRANT_PATH, post(pushBody(warrantFor("nas")))))!;
    expect(res.status).toBe(401);
    expect(h.writes()).toBe(0);
  });
  ...
  test("a GET on the path falls through to the ordinary 404 — the route is POST-only", async () => {
    const h = harness(peerStore());
    const res = (await call(asLead(h), CREW_WARRANT_PATH, { headers: authed }))!;
    expect(res.status).toBe(404);
  });
});
...
describe("what a member reports about the warrant it holds (§18, RFC §11.2)", () => {
  ...
  test("snapshot carries them BESIDE the body, never inside the browser's own shape", async () => {
    ...
    // mutated by being read for the crew: a crew-only field never leaks into the local snapshot.
    expect(body).toEqual(ownSnapshot());
  });
  ...
  test("hello OMITS the activation when nothing is active — absent, never zero (§7.1)", async () => {
    ...
    const handler = createCrewRouter({ store: h.store, audit: h.audit, now: () => T0, warrantActiveGeneration: null });
    const res = (await call(handler, CREW_HELLO_PATH, {
      headers: { ...authed, ...signed("nas", "GET", CREW_HELLO_PATH, "", T0) },
    }))!;
    expect(await res.json()).toEqual({ protocol: 1, member: "desk", warrantGeneration: 1, warrantRefreshedAt: T0 });
...
describe("Gap A — a peer knows when its lead last called (§18.9)", () => {
  ...
  test("a refusal on the SECRET is recorded separately — §8.4's rotation, from the dropped side", async () => {
    ...
    // longer hold the crew secret" — the fact that lets RFC §8.3's *stranded by a rotation* be
...
describe("a member that speaks to its lead is due (M20/02)", () => {
  test("every ADMITTED call from a roster member is contact, on every route it lands on", async () => {
    ...
    await call(handler, CREW_HELLO_PATH, { headers: { ...authed, ...signed("nas", "GET", CREW_HELLO_PATH, "", T0) } });
    await call(handler, CREW_HELLO_PATH, { headers: { ...authed, ...signed("nas", "GET", CREW_HELLO_PATH, "", T0 + 1) } });
    expect(contacted).toEqual(["nas", "nas"]);
  });

  test("GUARD ONE: a caller refused on either factor is not contact", async () => {
    // The whole safety of the reset rests here. `admitCrewRequest` runs first, so a caller that
...
describe("Gap B — lead_conflict, the named answer (§18.10)", () => {
  ...
  test("this collie's own lead is never told it conflicts", async () => {
    const h = harness(rePinned());
    const handler = createCrewRouter({ store: h.store, audit: h.audit, transportPinned: true });
    const res = (await call(handler, CREW_HELLO_PATH, { headers: dialledBy("nas") }))!;
    expect(res.status).toBe(200);
    // …and neither is a caller that named nobody: an absent hint is not a claim to disagree with.
    expect((await call(handler, CREW_HELLO_PATH, { headers: authed }))!.status).toBe(200);
  });
  ...
  test("a LEAD never answers it — its members are each a legitimate caller", async () => {
    ...
    const handler = createCrewRouter({ store: h.store, audit: h.audit, now: () => T0 });
    const res = (await call(handler, CREW_HELLO_PATH, {
      headers: { ...dialledBy("nas"), ...signed("nas", "GET", CREW_HELLO_PATH, "", T0) },
    }))!;
    expect(res.status).toBe(200);
  });
  ...
  test("it refuses BEFORE dispatch — a member that follows somebody else is answered, not served", async () => {
    ...
    const res = (await call(handler, CREW_SNAPSHOT_PATH, { headers: dialledBy("desk") }))!;
    expect(res.status).toBe(409);
    expect(h.writes()).toBe(0);
  });
});
...
describe("POST /pack/v1/warrant at a collie that still believes it leads — the deposition (§18.12)", () => {
  ...
  test("the new lead hands the warrant back, and the old one self-heals to a peer of it", async () => {
    ...
    const seen: unknown[] = [];
    const body = { warrant };
    const res = (await call(h.store && router(h, seen), CREW_WARRANT_PATH, signedPost("nas", CREW_WARRANT_PATH, body, T0 + 100)))!;
    expect(res.status).toBe(200);
  ...
  test("a member that is NOT the one the warrant names proves nothing — a warrant is public", async () => {
    ...
    const { data, warrant } = stale();
    const h = harness(data);
    const res = (await call(router(h), CREW_WARRANT_PATH, signedPost("attic", CREW_WARRANT_PATH, { warrant }, T0 + 100)))!;
    expect(res.status).toBe(401);
    expect(h.data().lead).toBeNull();
  });
  ...
  test("a warrant this collie did NOT sign is the uniform 401, and the store is untouched", async () => {
    ...
    const res = (await call(router(h), CREW_WARRANT_PATH, signedPost("nas", CREW_WARRANT_PATH, { warrant: forged }, T0 + 100)))!;
    expect(res.status).toBe(401);
  ...
  test("a deputy the roster cannot resolve PARKS — announced, and nothing is written", async () => {
    ...
    // hand-edited store or a crew it does not belong to. `nas` is still pinned and still signs
    ...
    const h = harness(data);
    const seen: unknown[] = [];
    const res = (await call(router(h, seen), CREW_WARRANT_PATH, signedPost("nas", CREW_WARRANT_PATH, { warrant }, T0 + 100)))!;
  ...
  test("an UNSIGNED push cannot depose: a lead's listener pins nothing inbound (§8.1)", async () => {
    ...
    const { data, warrant } = stale();
    const h = harness(data);
    const res = (await call(router(h), CREW_WARRANT_PATH, post({ warrant })))!;
    expect(res.status).toBe(401);
    expect(h.data().lead).toBeNull();
  });
  ...
  test("a PEER is unaffected: the same route still stores rather than deposes", async () => {
    ...
    const w = mintWarrant(leadStore({ peers: [member({ memberId: "nas" })] }), "nas", T0)!.result;
    const handler = createCrewRouter({ store: h.store, audit: h.audit, transportPinned: true, now: () => T0 });
    const res = (await call(handler, CREW_WARRANT_PATH, post({ warrant: w, deputyCertPem: material("nas").certPem })))!;
    expect(await res.json()).toEqual({ generation: 1, applied: true });
  });
});
...
describe("a two-anchored peer requires an attested dial, and gives the deputy ZERO reach (§8.1)", () => {
  ...
  const EVERY_ROUTE: readonly (readonly [string, string])[] = [
    [CREW_HELLO_PATH, "GET"],
    [CREW_SNAPSHOT_PATH, "GET"],
    [CREW_SECRET_PATH, "POST"],
    [CREW_WARRANT_PATH, "POST"],
    [CREW_LEAD_PATH, "POST"],
    [CREW_LEAVE_PATH, "POST"],
    [`${CREW_PREFIX}pane/w1:p1`, "GET"],
  ];
  ...
  test("a SINGLE-anchor peer still admits an unattested dial — today, byte for byte", async () => {
    const h = harness(peerStore());
    const res = (await call(router(h), CREW_HELLO_PATH, { headers: authed }))!;
    expect(res.status).toBe(200);
  });
  ...
  test("a DEPUTY-attested dial reaches ONLY the two routes DEPUTY_ROUTES names", async () => {
    ...
    const { data, deputyAnchor } = anchored();
    const reachable = new Set<string>([CREW_TAKEOVER_PATH, CREW_WARRANT_PATH]);
    for (const [path, method] of EVERY_ROUTE) {
  ...
  test("a DEPUTY-attested dial with the WRONG METHOD is refused, on both of its own routes", async () => {
    const { data, deputyAnchor } = anchored();
    for (const path of [CREW_TAKEOVER_PATH, CREW_WARRANT_PATH]) {
      const h = harness(data);
      const res = (await call(router(h, deputyAnchor), path, {
  ...
  test("a TAMPERED attestation is refused — the signature is over the method, path and receiver", async () => {
    ...
    for (const bent of [
      ...
      attested("desk", "GET", CREW_SNAPSHOT_PATH),
      // Signed for another method.
      attested("desk", "POST", CREW_HELLO_PATH),
      ...
      attested("desk", "GET", CREW_HELLO_PATH, T0, "nas"),
      // Signed by a member this peer anchors neither of.
      attested("attic", "GET", CREW_HELLO_PATH),
      // Re-stamped: the timestamp is inside the signature, so it cannot be walked forward.
      { ...attested("desk", "GET", CREW_HELLO_PATH), [TIMESTAMP_HEADER]: String(T0 + 1) },
      // Truncated to nothing.
      { [DIAL_HEADER]: "", [TIMESTAMP_HEADER]: String(T0) },
    ]) {
      const h = harness(data);
      const res = (await call(router(h, deputyAnchor), CREW_HELLO_PATH, { headers: { ...authed, ...bent } }))!;
      expect(res.status).toBe(401);
    }
  });
  ...
  test("a STALE attestation is refused — §8.6's skew window, unchanged", async () => {
    ...
    const res = (await call(router(h, deputyAnchor), CREW_HELLO_PATH, {
      headers: { ...authed, ...attested("desk", "GET", CREW_HELLO_PATH, old) },
    }))!;
    expect(res.status).toBe(401);
    // Inside the window it is fine, so what refused it was the clock and not the shape.
    const fresh = (await call(router(h, deputyAnchor), CREW_HELLO_PATH, {
      headers: { ...authed, ...attested("desk", "GET", CREW_HELLO_PATH, T0 - MAX_SKEW_MS + 1) },
    }))!;
  ...
  test("CONCURRENT attested dials all land — the replay FLOOR is not applied here", async () => {
    ...
    const headers = { ...authed, ...attested("desk", "GET", CREW_HELLO_PATH) };
    for (let i = 0; i < 3; i++) {
      expect((await call(handler, CREW_HELLO_PATH, { headers }))!.status).toBe(200);
    }
  });
...
describe("POST /pack/v1/pairing — the lead syncs its registry to the DEPUTY only (RFC §6.5)", () => {
  ...
  /** A peer of `desk` holding the warrant that names IT — i.e. this crew's deputy. */
  function deputy() {
  ...
  test("a build with no standby surface at all refuses it — absent means closed", async () => {
    ...
    const handler = createCrewRouter({ store: h.store, audit: h.audit, transportPinned: true, now: () => T0 });
    expect((await call(handler, CREW_PAIRING_PATH, post(body())))!.status).toBe(401);
  });
  ...
  test("a malformed body is a 400 on an admitted link, and syncs nothing", async () => {
    const r = router(harness(deputy()));
    expect((await call(r.handler, CREW_PAIRING_PATH, post({ devices: [] })))!.status).toBe(400);
    expect(r.synced).toEqual([]);
  });
  ...
  test("a LEAD never answers it: it has no lead of its own to have sent it", async () => {
    const h = harness(leadStore({ peers: [member({ memberId: "nas" })] }));
    const r = router(h);
    expect((await call(r.handler, CREW_PAIRING_PATH, post(body())))!.status).toBe(401);
  });
});
...
describe("POST /pack/v1/takeover — the witness question and the re-pin (RFC §7)", () => {
  ...
  const asDeputy = <TBody,>(body: TBody, label = "nas", path = CREW_TAKEOVER_PATH): RequestInit => ({
    method: "POST",
  ...
  test("PROBE while the lead is quiet: a witness, and NOTHING is written", async () => {
    ...
    const h = harness(data);
    const before = h.writes();
    const res = (await call(router(h, deputyAnchor), CREW_TAKEOVER_PATH, asDeputy({ warrant })))!;
    expect(res.status).toBe(200);
  ...
  test("PROBE while the lead is calling: lead_is_alive — the answer that aborts a takeover", async () => {
    const { data, warrant, deputyAnchor } = anchoredPeer();
    const h = harness(data);
    const res = (await call(router(h, deputyAnchor, 2000), CREW_TAKEOVER_PATH, asDeputy({ warrant })))!;
  ...
  test("an ABSENT phase is a probe: a commit must be asked for explicitly", async () => {
    const { data, warrant, deputyAnchor } = anchoredPeer();
    const h = harness(data);
    const res = (await call(router(h, deputyAnchor), CREW_TAKEOVER_PATH, asDeputy({ warrant, address: "nas:1" })))!;
  ...
  test("a COMMIT with no address is a 400 — a peer must know where to dial its new lead", async () => {
    ...
    const h = harness(data);
    const before = h.writes();
    const res = (await call(router(h, deputyAnchor), CREW_TAKEOVER_PATH, asDeputy({ warrant, phase: "commit" })))!;
    expect(res.status).toBe(400);
    expect(h.writes()).toBe(before);
  });
  ...
  test("a BAD SIGNATURE is the uniform 401, not a sentence", async () => {
    ...
    const res = (await call(router(h, deputyAnchor), CREW_TAKEOVER_PATH, asDeputy({ warrant: bent })))!;
    expect(res.status).toBe(401);
    expect(await res.json()).toEqual({ error: "unauthorized" });
  });
  ...
  test("an UNATTESTED dial never reaches the route at all", async () => {
    const { data, warrant, deputyAnchor } = anchoredPeer();
    const h = harness(data);
    const res = (await call(router(h, deputyAnchor), CREW_TAKEOVER_PATH, post({ warrant })))!;
    expect(res.status).toBe(401);
  });
  ...
  test("a MEMBER (this peer's own lead) is refused on it — the route is the deputy's", async () => {
    ...
    const res = (await call(
      router(h, deputyAnchor),
      CREW_TAKEOVER_PATH,
      { ...asDeputy({ warrant }, "desk"), headers: { ...authed, "content-type": "application/json", ...dial("desk", CREW_TAKEOVER_PATH) } },
    ))!;
    expect(res.status).toBe(401);
  });
  ...
  test("RFC §9's reconciliation: the same decision, on /pack/v1/warrant, from a deputy-admitted caller", async () => {
    ...
    const res = (await call(
      router(h, deputyAnchor),
      CREW_WARRANT_PATH,
      asDeputy({ warrant, address: "nas.example:8787" }, "nas", CREW_WARRANT_PATH),
    ))!;
    expect(res.status).toBe(200);
  ...
  test("at a collie that still LEADS, a probe answers lead_is_alive and a commit deposes it (§18.12)", async () => {
    ...
    const signedTakeover = <TBody,>(body: TBody) => signedPost("nas", CREW_TAKEOVER_PATH, body, T0 + 1);
    ...
    const probed = (await call(
      createCrewRouter({ store: probe.store, audit: probe.audit, now: () => T0 + 1 }),
      CREW_TAKEOVER_PATH,
