...
describe("minting", () => {
  ...
  test("refuses a member behind on the crew secret", () => {
    const stale = lead({ peers: [member({ memberId: "nas", secretGeneration: CREW.secretGeneration - 1 })] });
...
describe("refresh (RFC §4.5)", () => {
  ...
  test("REFUSES to refresh an expired warrant — a dark crew disarms rather than re-arming", () => {
    const { data } = named();
...
describe("supersession and TTL", () => {
  ...
  test("expiry is 30 days from the last refresh, on the reader's own clock", () => {
    ...
    // last time the crew was healthy.
...
describe("a warrant from a crew this collie is not in (the incident)", () => {
...
test("a follow-driven restart writes no warrant field — the update path never touches the store", async () => {
  ...
  // follow path can write the field. `bridge/crew/follow.ts` reads a run record and spawns a
