...
import type { CrewLink, PeerFailure, PeerOutcome } from "./peer-client.ts";
...
//
// An address comes from the trust store or from nowhere: {@link CrewRegistry.resolve} looks a host up
 ...
 * nothing about the version" — see {@link CrewRegistry.record}.
 */
 ...
 *   belongs to another crew (§18.10), a member that said no (§14.3), or a budget that is spent.
 *
 ...
 * the split is decided — a second site would let the chip and the crew page disagree about one
 * machine.
 *
...
export function linkStateFor(state: PeerFailure["state"], authRefused: boolean, runs: number): PeerLinkState {
  ...
  // crew's member and a flat "no" are all things a machine said, and re-dialling says it again.
...
/** The lead's belief about one peer — everything `crew status` and the `servers` array render. */
export interface PeerState {
   ...
   *
   * A member can be reachable and still carry one: {@link CrewRegistry.recordProbe}'s slow-link note
   ...
   * (§10.4), so a version that rode `hello` alone stayed `null` on every healthy crew — which is
   ...
   * machine has not heard about, and a lower one is a peer that has not caught up. `crew status`
 ...
 * different in kind (a member has moved crews; no amount of waiting fixes it).
 */
...
export type HostResolution =
  ...
  | { readonly kind: "peer"; readonly link: CrewLink; readonly state: PeerState };

export interface CrewRegistryDeps {
...
export class CrewRegistry {
  ...
  constructor(private readonly deps: CrewRegistryDeps) {}
  ...
  record(memberId: string, outcome: PeerOutcome<unknown>, observed?: PeerObservation): PeerState {
    ...
    const next: PeerState = outcome.ok
      ...
      : {
          ...
          // "this peer belongs to someone else's crew now" as "the laptop is shut". §10.2's three
   ...
   *
   * Two rules separate it from {@link CrewRegistry.record}, and both exist because a probe learns
   ...
   * and every phone write to one is an authenticated round trip over the same crew link (§5, §9.1),
...
function linkFor(member: TrustedMember): CrewLink {
