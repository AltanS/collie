...
import { leadStore, member, CREW, T0 } from "./fixtures.ts";
import { runBootGate, type BootGateDeps } from "./boot-gate.ts";
import { isDepositionProof } from "./deposed.ts";
import type { HelloResult, CrewLink, PeerOutcome } from "./peer-client.ts";
...
describe("the boot gate (§18.11)", () => {
  ...
  test("a conflict with NO warrant does NOT depose — it warns, and the lead keeps leading", async () => {
    ...
    // stale file into the whole crew's outage.
  ...
  test("a conflict whose warrant belongs to ANOTHER crew warns, and names the remedy", async () => {
    const answer = await gate({ nas: conflicted(FOREIGN) }).verdict;
    expect(answer.kind).toBe("publish");
    if (answer.kind !== "publish") return;
    expect(answer.warnings[0]).toContain('crew "crew-elsewhere"');
    expect(answer.warnings[0]).toContain("collie crew leave");
  });

  test("a member holding a HIGHER generation warns rather than deposing", async () => {
    // This is the exact shape of the incident: a peer that kept generation 3 from the crew it left,
