...
// investigation is recorded in `.tracker/M4-crew-federation-engine/08-*.md`.
//
...
// (`CrewRequestFacts.transportPinned`, bridge/crew/admission.ts). Everything else follows:
//
...
//     most trust.** A crew that names none is unaffected, byte for byte.
//   • A LEAD does not pin its listener at all. Its crew surface rides the front door, and
...
//     the restart every membership verb already performs (`applyLocally` in cli/crew.ts), which is
//     also why they perform it.

/** TLS options for a listener or a dial. Structural on purpose — Bun's and Node's shapes both fit. */
export interface CrewTlsOptions {
...
export type CrewRequestInit = RequestInit & { tls?: CrewTlsOptions };
 ...
 * `transportPinned: false` to the router, admission then refuses every request, and the crew is down
 ...
 *
 *   • the warrant must be for THIS crew and issued by THIS peer's lead — a warrant from anywhere else
 ...
 *   • it must not be past its 30 days on THIS machine's clock (§18.4). A dark crew disarms itself,
 ...
 */
export function deputyAnchorOf(mode: CrewMode, data: TrustStoreData | null, now: number = Date.now()): string | null {
  if (mode !== "peer" || data === null) return null;
  const lead = data.lead;
  if (lead === null || lead.status !== "enrolled" || lead.certPem === "") return null;
 ...
 */
export function dialTls(data: TrustStoreData | null, member: Pick<TrustedMember, "certPem">): CrewTlsOptions | null {
