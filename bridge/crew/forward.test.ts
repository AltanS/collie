...
describe("which routes cross a link", () => {
  test("the pane family, tabs and workspace map 1:1 onto the crew prefix", () => {
    expect(crewRouteFor("/api/pane/w1:p1")).toBe("pane/w1:p1");
    for (const action of ["reply", "keys", "upload", "close", "rename", "history"]) {
      expect(crewRouteFor(`/api/pane/w1:p1/${action}`)).toBe(`pane/w1:p1/${action}`);
    }
    expect(crewRouteFor("/api/tab")).toBe("tab");
    expect(crewRouteFor("/api/tab/w1:t1/rename")).toBe("tab/w1:t1/rename");
    expect(crewRouteFor("/api/tab/w1:t1/close")).toBe("tab/w1:t1/close");
    expect(crewRouteFor("/api/workspace")).toBe("workspace");
  });
  ...
  test("launch and its rows cross the link too — rows must come from the host that runs them", () => {
    ...
    expect(crewRouteFor("/api/launchers")).toBe("launchers");
  ...
  test("no route can escape the prefix by shape", () => {
    expect(crewRouteFor("/api/pane/w1:p1/../../etc/passwd")).toBeNull();
    expect(crewRouteFor("/api/pane/a/b/c")).toBeNull();
  ...
  test("the grammar matches server.ts's own route literals, character for character", () => {
    ...
    for (const action of tabActions) expect(crewRouteFor(`/api/tab/x/${action}`)).toBe(`tab/x/${action}`);
...
describe("a forwarded write carries its own budget (§10.1)", () => {
  ...
  test("the write budget outlasts the poll budget it replaced", () => {
    ...
    expect(WRITE_BUDGET_MS).toBeGreaterThan(crewTimeoutBudget(1500, {}));
  });
  ...
  function budgeted(workMs: number) {
    ...
    const transport: ForwardTransport = async (_link, route, _params, _init, budgetMs) => {
      const deadline = budgetMs ?? crewTimeoutBudget(1500, {});
      calls.push(deadline);
...
describe("journal, uploads and state stay host-local", () => {
  ...
  test("four mux-port verbs have no forwardable crew route, and the contract says which", () => {
    ...
    for (const route of unreachable) {
      expect(crewRouteFor(`/api/${route}`)).toBeNull();
