...
// parks the result as *unverifiable*. A real crew showed what that costs. A peer left crew A while
// holding a generation-3 warrant, kept the fields, joined crew B, and the new lead's next boot read
// generation 3 > 0, parked itself `no-proof`, tore down its front door and went dark. Nothing had
// happened to the crown: one stale field on one machine took the crew offline, and recovery was by
...
// turns one peer's stale file into the crew's outage, which is a worse failure than the split brain
...
export interface BootGateDeps {
  ...
  /**
   * `PeerClient.hello`, on the patient budget (§10.4). Injected for the reason every other crew
   * transport is: the decision has to be exercisable without a socket.
   */
  readonly hello: (link: CrewLink) => Promise<PeerOutcome<HelloResult>>;
...
export async function runBootGate(deps: BootGateDeps): Promise<BootGateVerdict> {
  ...
  for (const link of deps.links) {
    ...
    // else — OR it is carrying a field out of a crew it used to belong to, which is the case that
    // took a real crew dark. It carries no warrant either way, so it is a claim and it is logged.
    ...
    if (reported !== null && reported > deps.generation) {
      warnings.push(
        `"${link.memberId}" reports warrant generation ${reported}, which this lead never minted; ` +
          "ignoring it. If that machine came from another crew, run `collie crew leave` there and re-join it.",
      );
    }
  }
 ...
 *
 * A warrant stamped with ANOTHER crew's id gets its own sentence, because it names the remedy: that
 * machine did not leave its old crew cleanly, and no amount of restarting this one will fix it.
 */
