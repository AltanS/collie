...
import { parseRoster, type CrewChange } from "./enrollment.ts";
 ...
 * time the crew was healthy — a crew in daily use never approaches it, and a crew that has been dark
...
/**
 * `collie crew deputy <member>` — mint generation N+1 naming `deputyMemberId`, or naming **nobody**
 * when it is `null` (RFC §4.4's revocation).
 *
 * Refuses (`null`) unless this collie leads a crew and the named member is an **enrolled** peer of
 * its own roster holding the current secret generation — the same validation `crew approve-promote`
...
export function mintWarrant(
  ...
  if (deputyMemberId !== null) {
    ...
    if (member === undefined || member.secretGeneration !== crew.secretGeneration) return null;
    ...
    // warrant to mint here — the crew already names nobody (RFC §4.4).
    return null;
  }
  ...
  return {
    ...
    // designated crew still explain itself with a takeover that is now history.
    next: {
      ...data,
 ...
 * re-signing it here would silently re-arm a crew that has been dark for a month without the operator
 * doing anything. Re-running `crew deputy` is the way back, and it is one verb.
 */
...
export interface WarrantPush {
   ...
   * lead a crew it cannot dial and holds exactly one roster entry of its own, so the alternative to
   * this field is a takeover into a crew the new lead cannot see. It is the identical payload
   * §14.3's successful demotion already returns, for the identical reason.
   *
   * **Not signed, and it does not need to be**: it arrives over a two-factor crew link from the
...
export type WarrantRefusal =
  ...
  | "malformed"
  /** A warrant for another crew, or one this collie's own lead did not issue. */
  | "foreign"
...
/**
 * Belt and braces for the field that took a real crew dark: **a warrant stamped with another crew's
 * id is not this collie's warrant, so it is discarded.**
 *
 * `leaveCrew` now clears these fields, so a machine that leaves cleanly never reaches this. What
 * reaches it is a store written by an older build, or a `join` that landed beside a warrant nobody
 * cleared. Either way the fields describe a crew this collie is not in, and the cost of keeping them
 * is that this machine reports a generation its own lead never minted — which is read as a takeover
 * on the far end.
 *
 * `null` — nothing to write — when there is no crew, no warrant, or the warrant is this crew's.
 * `standbyRoster` and `deputy` go with it: all three were written by the same push, from the same
 * lead, about the same crew.
 */
...
export function storeWarrant(
   ...
   * running an older build — neither is evidence the crew shrank to nothing.
   */
