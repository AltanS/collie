...
// as success. A crew link is `.adr/0010`'s problem over a lossier wire — a retry types the bytes into
// a real terminal twice.

/**
 * The crew route for a local `/api/...` pathname, or `null` when this route is not forwardable.
 *
 ...
 * by simply not appearing: push subscriptions live on the lead, notification policy is one crew-wide
 ...
 */
export function crewRouteFor(pathname: string): string | null {
 ...
 * unaudited basis for a decision the crew link has already made (§6, ADR 0013).
 *
 ...
 *
 * The health here is what the lead's poll last observed (bridge/crew/registry.ts) — so the common
...
export function refuseWriteBeforeAttempt(state: PeerState): Response | null {
  ...
  if (state.health === "incompatible") {
    return forwardError(
      "host_incompatible",
      `host ${state.memberId} speaks a crew protocol this build cannot: ${state.reason ?? "version mismatch"}`,
      503,
 ...
 * a fault that provably never left this process (`attempted === false` — no crew secret, unusable
...
/**
 * The transport, narrowed to what forwarding needs — {@link CrewLead.forward} in production.
 *
 * `budgetMs` is the deadline for THIS dial, and it is present for exactly one shape of request: a
 * forwarded write, on {@link WRITE_BUDGET_MS} (§10.1). Absent means "the transport's own default",
 * which for `PeerClient.proxy` is the poll budget plus a cold link's bootstrap credit — i.e. every
 * forwarded read keeps the budget it has always had.
 */
...
export interface ForwardDeps {
   ...
   * is, and `CrewRegistry.recordExchange` folds it into `lastSeenAt` so the freshness a phone reads
...
export async function forwardToPeer(req: Request, url: URL, deps: ForwardDeps): Promise<Response> {
  const route = crewRouteFor(url.pathname);
  if (route === null) {
    ...
    return forwardError("route_not_federated", `route ${url.pathname} is not available across a crew link`, 501, {
  ...
  // two different numbers is therefore legal and merely confusing; keep a crew on one.
  ...
  // ── A WRITE IS NOT A POLL ────────────────────────────────────────────────────
  // A read rides the sweep's budget (§10.1): it asks a peer for bytes it already holds, and a slow
  // one must not stall the lead's own snapshot. A write asks the peer to DO something — a launch
  // spawns a process and has a multiplexer build a tab — and 1200 ms of that is a coin flip. Measured
