 ...
 *
 * **Called on exactly one path** — `ensureStore` in `cli/crew.ts`, at the operator's first
 * `crew invite` or `join`. A solo instance never mints, never writes a key and never has a trust
...
/**
 * Mint a single-use, short-lived enrollment token, creating the crew if this is its first invite.
 *
 * The crew coming into existence here rather than at startup is the zero-tax rule in practice: a
 * lead that never invites anybody never has a crew identity, never has a secret, and therefore has
 * nothing to write. Minting an invite is the operator's first crew action, so it is the moment the
 * crew secret is generated.
 *
 ...
 * full re-serialize of the file holding the private key and the crew secret, plus an audit line,
 * once per request. The refusal the caller sees is identical either way: `commitCrewChange`
...
export function consumeInvite(
  ...
  token: string | null,
  now: number,
): CrewChange<PendingInvite | null> | null {
...
export function enrollPeer(
  ...
  const member: TrustedMember = {
    ...
    signedAt: 0,
    // A freshly-enrolled member starts provisional: pinned, but never once contacted. `crew status`
...
/**
 * Peer side: adopt the crew the lead just handed over, pin the lead, and take the id it minted.
 *
...
/**
 * Reissue the crew secret (§8.4). **No grace window, no rollback secret** — the old value stops being
 ...
 */
export function rotateCrewSecret(
  data: TrustStoreData,
  now: number,
  random: RandomSource = randomToken,
 ...
 *
 * They are marked, not deleted, so `crew status` can say *why* a machine went quiet and the operator
...
/**
 * `collie crew remove <member>` on the lead: unpin and forget.
 *
...
export function removeMember(
  data: TrustStoreData,
  memberId: string,
): CrewChange<{ member: string; deputy: boolean }> | null {
  if (!data.peers.some((p) => p.memberId === memberId)) return null;
  // ── A DISOWNED MACHINE IS NOT STILL THE DEPUTY ──────────────────────────────
  // `crew status` reads the DESIGNATION (`cli/crew-status-deputy.ts`), and `crew deputy --revoke`
  // reads the roster, so removing the named deputy used to leave the two disagreeing out loud:
  // status printed `deputy <member> — warrant generation N` while `--revoke` answered "this crew
  ...
  //
  // The WARRANT stays. It carries the generation counter, which must never reset inside a crew
  // (§18.3, RFC §4.4) — dropping it would let an old warrant of this crew verify again. It names a
...
/**
 * `collie leave` on a peer: drop the roster entry, the pinned material, the crew secret **and every
 * deputy field the old crew wrote here**.
 *
 ...
 * lead's `crew remove` and this are independent, and a lost disk on one end is handled from the
 * other.
 *
 ...
 * crew found what that costs. An armed deputy at generation 3 left crew A, joined crew B, and kept
 * reporting generation 3 on `hello`. Crew B's brand-new lead, which had never minted a warrant, read
 ...
 *
 * The generation counter's "never resets" rule (§18.3, RFC §4.4) is scoped to a crew, and this
 * machine is leaving one. A counter carried across that boundary is not a defence against replay,
 * it is a claim about a crew this collie no longer belongs to — which is exactly the claim that did
...
//
// Everything below is driven by an OPERATOR VERB on one machine and lands on another over the crew
// link (`bridge/crew/router.ts`'s `secret` / `lead` / `leave` routes). They are transitions like the
 ...
 * one. That ordering is the distributing verb's job (`collie crew rotate` rotates locally FIRST and
...
/**
 * Peer side: `lead` is the crew's lead from now on (§14) — re-pin it and dial it there instead.
 *
 * **A role change, not a re-enrollment**: the crew identity, the crew secret and this collie's own
...
/**
 * `collie crew approve-promote <member>` — arm the ten-minute, single-use consent (§14.1).
 *
...
/**
 * `collie crew approve-promote --cancel` — the operator armed it and changed their mind (§14.1).
 *
 ...
 */
export function cancelPromotion(data: TrustStoreData, now: number): CrewChange<PendingHandover> | null {
...
export function isDemotionRefused(
  outcome: CrewChange<{ roster: RosterEntry[] }> | DemotionRefused | null,
 ...
 *     agreed, and without this clause one compromised peer takes the crew (§8.5's F2);
 ...
 * passes an empty list — the crew the operator can still reach is the crew they get, and every member
...
export function recordSignedRequest(
  ...
  memberId: string,
  timestamp: number,
): CrewChange<{ signedAt: number }> | null {
 ...
 */
export async function commitCrewChange<T>(
  store: TrustStore,
  audit: AuditLog | null,
  fn: (data: TrustStoreData | null) => CrewChange<T> | null,
): Promise<T | null> {
