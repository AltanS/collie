...
import { counterRandom, fp, leadStore, material, member, CREW, peerStore, T0 } from "./fixtures.ts";
...
describe("this collie's own identity", () => {
  ...
  test("a fresh store is an identity and nothing else — no crew, no roster, no invites", () => {
...
describe("spending an invite", () => {
  ...
  test("a wrong or absent token against only-live invites is a NO-OP — no store to write, nothing spent", () => {
    ...
    // enrollment endpoint re-serialize the file holding the private key and the crew secret.
...
describe("the exchange — §8.2's transfer table, both directions", () => {
  ...
  test("the peer adopts the crew, pins the lead, and takes the id the lead minted", () => {
...
describe("the exchange — parsing untrusted payloads", () => {
  ...
  test("a response with an out-of-grammar member id or unpinnable fingerprint is refused", () => {
    ...
    expect(parseEnrollResponse({ ...res, crewSecret: 7 })).toBeNull();
...
describe("rotation (§8.4)", () => {
  ...
  test("every member is left behind until it picks the secret up", () => {
    const rotated = rotateCrewSecret(withPeers, T0 + 10, R())!.next;
  ...
  test("a fully caught-up crew drops nobody", () => {
...
describe("revocation (§8.4)", () => {
  test("`crew remove` deletes the entry — a disowned machine keeps no pin", () => {
  ...
  // printed a deputy while `crew deputy --revoke` answered "this crew names no deputy".
  test("removing the DEPUTY drops the designation, and keeps the warrant's counter", () => {
    ...
    // The counter never walks backwards inside a crew (§18.3), so the warrant itself stays put.
  ...
  // `deputy`, `warrant` and `standbyRoster`. The peer then joined another crew, reported generation
  // 3 there, and that crew's brand-new lead parked itself over a warrant it had never minted.
  test("`leave` clears every deputy field — a warrant belongs to the crew that signed it", () => {
    ...
    const next = leaveCrew(armed)!.next;
    ...
    // And nothing of the old crew survives in the bytes at all.
  ...
  test("after `leave` the mode falls back to solo without deleting the file", () => {
    const left = leaveCrew(peerStore())!.next;
    expect(left.lead).toBeNull();
    expect(left.peers).toEqual([]);
  });
});

describe("commitCrewChange — write first, audit second", () => {
  ...
  test("a successful change is persisted and audited", async () => {
    ...
    // The result distinguishes "changed" from "no-op": `commitCrewChange` answers `null` for the
    // latter, so a transition whose result was also `null` would be indistinguishable to a verb.
    expect(await commitCrewChange(store, h.audit, (d) => removeMember(d!, "nas"))).toEqual({
      member: "nas",
      deputy: false,
    });
  ...
  test("a no-op change writes nothing and audits nothing", async () => {
    ...
    const store = new TrustStore("/unused", h.io);
    expect(await commitCrewChange(store, h.audit, (d) => removeMember(d!, "ghost"))).toBeNull();
    await Bun.sleep(5);
  ...
  test("a change that fails to PERSIST is never audited — the log must not claim a write that lost", async () => {
    ...
    await expect(commitCrewChange(store, h.audit, (d) => removeMember(d!, "nas"))).rejects.toThrow("disk full");
    await Bun.sleep(5);
    expect(h.lines).toEqual([]);
  });
  ...
  test("rotation and revocation both reach the audit log through the same path", async () => {
    const h = harness(leadStore({ peers: [member({ memberId: "nas" })] }));
    const store = new TrustStore("/unused", h.io);
    await commitCrewChange(store, h.audit, (d) => rotateCrewSecret(d!, T0, R()));
...
// The transitions the operator verbs in `cli/crew.ts` commit. Same rule as everything above: pure in,
...
describe("adoptLead / demoteSelf / promoteSelf", () => {
  // `peerStore()` is "laptop", enrolled by "desk"; the crew is promoting "nas".
  const claim = { memberId: "nas", fingerprint: fp("nas"), certPem: material("nas").certPem, address: "nas.example:8787" };

  test("a peer re-pins the new lead and keeps its id, its crew and its secret — a role change", () => {
  ...
  test("the old lead steps down and hands over every enrolled peer except the new lead", () => {
    ...
    expect(JSON.stringify(change.result.roster)).not.toContain(CREW.secret);
  });
